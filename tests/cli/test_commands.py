...
from chartreux.cli.textual_ui.widgets.chat_input.input_kinds import Prompt, classify
from chartreux.cli.textual_ui.widgets.chat_input.text_area import ChatTextArea
from chartreux.core.skills.builtins.chartreux import SKILL as VIBE_SKILL

_SKILL_MODULE = "chartreux/core/skills/builtins/chartreux.py"
...
class TestCommandRegistry:
    ...
    def test_ampersand_is_plain_prompt_text(self, text: str) -> None:
        registry = CommandRegistry()
        assert classify(
            text, commands=registry, resolve_skill=lambda _: None
        ) == Prompt(text)
        body = ChatInputBody(command_registry=registry)
        assert body._parse_mode_and_text(text) == (">", text)
        assert ChatTextArea(command_registry=registry).mode_characters == {"!", "/"}
    ...
    def test_removed_hosted_commands_are_absent(self, name: str) -> None:
        registry = CommandRegistry()
        registry.refresh(CommandContext())
        assert registry.get_command_name(f"/{name}") is None
        assert registry.parse_command(f"/{name}") is None
        assert not registry.has_command(name)
        assert f"/{name}" not in registry.get_help_text()
    ...
    def test_parse_command_rejects_removed_connector_alias(self) -> None:
        registry = CommandRegistry()
        assert registry.parse_command("/connectors filesystem") is None
    ...
    def test_mcp_command_description_surfaces_auth_subcommands(self) -> None:
        ...
        assert "http|streamable-http" not in command.description
        assert "--transport streamable-http" in command.description
    ...
    @pytest.mark.parametrize(
        "command_name", ["model", "thinking", "log-level", "proxy-setup", "theme"]
    )
...
class TestBuiltinSkillCommandDrift:
    ...
    def _all_aliases() -> set[str]:
        ...
        # `/paste-image` is macOS-only, but the skill documents its condition,
        # and the guard must hold on every platform.
        return {
            alias
