...
def question_app(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(
        "chartreux.cli.textual_ui.widgets.question_app._INPUT_GRACE_PERIOD_S",
...
class TestQuestionAppGracePeriod:
    def test_select_and_cancel_ignored_within_grace_period(
        ...
        with (
            patch("chartreux.cli.textual_ui.widgets.question_app.time") as mock_time,
    ...
    def test_navigation_works_during_grace_period(self, question_app: QuestionApp):
        with patch("chartreux.cli.textual_ui.widgets.question_app.time") as mock_time:
