...
def _source(
    ...
    *,
    ...
    return MCPSourceSummary(
        name=name, transport="stdio", status=status, tools=tools or [], error=error
    )
...
def test_source_option_ids_preserve_name() -> None:
    option_id = _source_option_id("mistral:search")

    assert _source_from_option_id(option_id) == "mistral:search"
...
def test_source_sorting_puts_nonempty_sources_first_and_sorts_each_group() -> None:
    sources = [
        _source("Zulu Empty", status=MCPSourceStatus.DISABLED),
        _source(
            "zulu tools",
            status=MCPSourceStatus.NEEDS_AUTH,
            tools=[MCPToolSummary(name="search")],
        ),
        _source("Alpha Empty", status=MCPSourceStatus.CONNECTED),
        _source(
            "alpha tools",
...
def test_filter_sources_fuzzy_matches_and_ranks_names() -> None:
    sources = [_source("Slack"), _source("Google Drive"), _source("GitHub")]
...
async def test_search_fuzzy_filters_sources_without_taking_initial_focus() -> None:
    app = MCPAppHarness(
        _state(_source("Google Drive"), _source("GitHub"), _source("Slack"))
    )
    ...
    async with app.run_test() as pilot:
        ...
        source_ids = [
            ...
            if option.id is not None and option.id.startswith("server:")
        ]
...
def test_list_view_sorts_sources_by_discovered_tools() -> None:
    app = MCPApp(
        _state(
            _source("server-empty"),
            _source("server-tools", tools=[MCPToolSummary(name="search")]),
            _source("server-empty-2"),
            _source("server-tools-2", tools=[MCPToolSummary(name="search")]),
        )
    )
...
def test_server_detail_shows_error_over_needs_auth() -> None:
    source = _source(
        "slack",
        status=MCPSourceStatus.NEEDS_AUTH,
...
def test_server_detail_shows_error_over_unavailable_status() -> None:
    source = _source(
        "slack",
...
async def test_a_rejected_toggle_is_shown_to_the_user_rather_than_raised() -> None:
    ...
    rejection = AppServerResponseError(
        ProtocolError(
            ...
            message="MCP configuration changed; reload and retry the toggle.",
        )
    )
    app.app_server.resources.mcp.toggle = AsyncMock(side_effect=rejection)
    message = MCPApp.MCPToggled(name="local", disabled=True)

    # Do
    await ChartreuxApp.on_mcpapp_mcptoggled(app, message)

    # Assert
    assert rejection.error.message in app.notify.call_args.args[0]
