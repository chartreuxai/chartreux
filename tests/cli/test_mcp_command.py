...
from chartreux.app_server import _mcp_auth, mcp_catalog
from chartreux.cli import entrypoint, mcp_command as mcp_cli
...
def test_mcp_add_targets_user_config_when_project_config_exists(
    ...
    project_config = tmp_working_directory / ".chartreux" / "config.toml"
    project_config.parent.mkdir()
    project_config.write_bytes(tomli_w.dumps({"theme": "dark"}).encode())
...
def test_mcp_add_oauth_logs_in_and_opens_browser(
    ...
    *Assert*: The CLI opens the URL and retains the canonical persisted source.
    ...
    async def fake_login(server, *, on_url, headers, check_current) -> None:
...
def test_mcp_add_oauth_keeps_config_after_login_failure(
    ...
    *Assert*: The command fails cleanly while the source remains persisted.
    ...
    # Prepare
    async def fail_login(server, *, on_url, headers, check_current) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "MCP HTTP URL must not include userinfo credentials",
        ),
    ],
)
...
def test_entrypoint_routes_mcp_command(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(sys, "argv", ["chartreux", "mcp", "add", "docs"])
    monkeypatch.setattr(
...
def test_help_advertises_mcp_command(
    ...
    assert exc_info.value.code == 0
    assert "chartreux mcp --help" in capsys.readouterr().out
