...
from chartreux.cli.textual_ui.app import ChartreuxApp, ChatScroll
from chartreux.cli.textual_ui.widgets.load_more import (
...
@pytest.fixture
def vibe_config(make_config) -> ChartreuxConfigSchema:
    return make_config(session_logging=SessionLoggingConfig(enabled=False))
...
async def _wait_until(pause, predicate, timeout: float = 2.0) -> None:
    ...
    while (time.monotonic() - start) < timeout:
        if predicate():
            return
        await pause(0.02)
    # Pilot.pause drains messages and may finish after the deadline.
    if predicate():
        return
...
@pytest.mark.asyncio
@pytest.mark.parametrize("ready_after_pause", [True, False])
async def test_wait_until_checks_predicate_after_pause_crosses_deadline(
    monkeypatch: pytest.MonkeyPatch, ready_after_pause: bool
) -> None:
    now = 0.0
    ready = False
    pause_calls = 0
    ...
    async def pause(delay: float) -> None:
        nonlocal now, ready, pause_calls
        assert delay == 0.02
        pause_calls += 1
        now = 3.0
        ready = ready_after_pause

    # Restore the real clock before yielding back to the event loop.
    with monkeypatch.context() as patch:
        patch.setattr(time, "monotonic", lambda: now)
        if ready_after_pause:
            await _wait_until(pause, lambda: ready)
        else:
            with pytest.raises(
                AssertionError, match="Condition was not met within the timeout"
            ):
                await _wait_until(pause, lambda: ready)
    assert pause_calls == 1


async def _wait_for_load_more(app: ChartreuxApp, pause) -> None:
    await _wait_until(
        pause, lambda: len(app.query(HistoryLoadMoreMessage)) == 1, timeout=5.0
    )


def _load_more_remaining(app: ChartreuxApp) -> int:
...
async def test_ui_mount_defers_history_resume(
    vibe_config: ChartreuxConfigSchema, monkeypatch: pytest.MonkeyPatch
) -> None:
