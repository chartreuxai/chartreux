...
async def _build_persistent_app() -> ChartreuxApp:
    # Checked saves require an accepted actual user source, not a verbatim fake.
    ...
    return build_test_chartreux_app(agent_loop=loop)
...
def test_parse_mcp_add_args_accepts_url_and_options() -> None:
    args = parse_mcp_add_args(
        ...
        "--transport streamable-http --no-login"
    )
...
async def test_mcp_add_saves_oauth_server_and_prints_next_steps(
    ...
    app = await _build_persistent_app()
    ...
    assert not [
        widget._error for widget in mounted_widgets if isinstance(widget, ErrorMessage)
    ]
...
@pytest.mark.asyncio
async def test_mcp_add_saves_streamable_http_transport(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    app = await _build_persistent_app()
    ...
    await app.prepare()
    await app._mcp_add(
        "https://mcp.example.com/mcp --transport streamable-http --no-login"
    )
...
async def test_mcp_add_help_states_oauth_only(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert any(
        ...
        and "--transport <streamable-http>" in widget._content
