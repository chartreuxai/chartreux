...
class TestSessionPickerLoading:
    @pytest.mark.asyncio
    async def test_loading_animation_starts_and_stops_when_sessions_arrive(
        self,
    ) -> None:
        ...
        async with PickerHostApp(picker).run_test() as pilot:
            await pilot.pause()
            spinner = picker.query_one(SpinnerText)
            assert picker.is_loading is True
            ...
            assert picker.is_loading is False
            assert spinner._timer is None
            assert picker.has_sessions is True
            assert picker.query_one("#sessionpicker-loading").display is False

    @pytest.mark.asyncio
    async def test_empty_and_loaded_states_are_distinguishable(self) -> None:
        picker = SessionPickerApp(sessions=[], latest_messages={}, loading=True)
    ...
    @pytest.mark.asyncio
    async def test_discovery_failure_stops_spinner_and_keeps_cancel_available(
        self,
    ) -> None:
        picker = SessionPickerApp(sessions=[], latest_messages={}, loading=True)
        ...
        async with PickerHostApp(picker).run_test() as pilot:
            await pilot.pause()
            spinner = picker.query_one(SpinnerText)
            picker.set_loading(False, error="network unavailable")
            await pilot.pause()

            assert picker.is_loading is False
            assert picker.load_error == "network unavailable"
            assert spinner._timer is None
            assert "network unavailable" in str(
                picker.query_one(
                    ".sessionpicker-loading-status", NoMarkupStatic
                ).content
            )
            picker.action_cancel()

    @pytest.mark.asyncio
    async def test_close_during_discovery_clears_loading_and_spinner(self) -> None:
        picker = SessionPickerApp(sessions=[], latest_messages={}, loading=True)
...
class TestSessionPickerPreviewDebounce:
    def test_rapid_highlights_only_post_the_final_preview(
        self,
        sample_sessions: list[SavedSessionSummary],
        sample_latest_messages: dict[str, str],
        ...
        timers: list[tuple[FakePreviewTimer, Callable[[], None]]] = []
        messages: list[SessionPickerApp.SessionHighlighted] = []
        monkeypatch.setattr(
            SessionPickerApp, "is_mounted", property(lambda _picker: True)
        )
        monkeypatch.setattr(
            picker,
            "set_timer",
            lambda _delay, callback: (
                timers.append((FakePreviewTimer(), callback)) or timers[-1][0]
            ),
        )
        monkeypatch.setattr(picker, "post_message", messages.append)

        picker._schedule_preview("session-a")
        picker._schedule_preview("session-b")
        for _timer, callback in timers:
            callback()

        assert timers[0][0].stopped is True
        assert [message.session_id for message in messages] == ["session-b"]
        assert picker._preview_timer is None
        assert picker._pending_preview_session_id is None

    @pytest.mark.asyncio
    async def test_real_highlight_events_debounce_to_final_preview(
        self,
        sample_sessions: list[SavedSessionSummary],
        sample_latest_messages: dict[str, str],
    ) -> None:
        ...
        class PreviewHostApp(App[None]):
            def __init__(self) -> None:
                super().__init__()
                self.preview_session_ids: list[str | None] = []

            def compose(self) -> ComposeResult:
                yield picker

            def on_session_picker_app_session_highlighted(
                self, message: SessionPickerApp.SessionHighlighted
            ) -> None:
                self.preview_session_ids.append(message.session_id)

        app = PreviewHostApp()
        async with app.run_test() as pilot:
            await pilot.pause(0.01)
            app.preview_session_ids.clear()

            option_list = picker.query_one(OptionList)
            assert option_list.highlighted_option is not None
            assert option_list.highlighted_option.id == "session-a"

            option_list.action_cursor_down()
            option_list.action_cursor_down()
            await pilot.pause(0.2)

            assert app.preview_session_ids == ["session-c"]
            last_id = app.preview_session_ids[-1]
            assert last_id is not None
            assert sample_latest_messages[last_id] == "Add unit tests for the API"
            assert option_list.highlighted_option is not None
            assert option_list.highlighted_option.id == "session-c"

    def test_cancel_unmount_and_error_stop_pending_preview_timers(
        self,
        sample_sessions: list[SavedSessionSummary],
        sample_latest_messages: dict[str, str],
        monkeypatch: pytest.MonkeyPatch,
        ...
        monkeypatch.setattr(
            picker,
            "set_timer",
            lambda _delay, _callback: timers.append(FakePreviewTimer()) or timers[-1],
        )
        monkeypatch.setattr(picker, "post_message", lambda _message: None)

        picker._schedule_preview("session-a")
        picker.action_cancel()
        picker._schedule_preview("session-b")
        monkeypatch.setattr(
            SessionPickerApp, "is_mounted", property(lambda _picker: False)
        )
        picker.set_loading(False, error="network unavailable")
        monkeypatch.setattr(
            SessionPickerApp, "is_mounted", property(lambda _picker: True)
        )
        picker._schedule_preview("session-c")
        picker.on_unmount()
    ...
    def test_rapid_selection_only_posts_one_message(
        self,
        sample_sessions: list[SavedSessionSummary],
        sample_latest_messages: dict[str, str],
        monkeypatch: pytest.MonkeyPatch,
        ...
        event = cast(OptionList.OptionSelected, FakeOptionEvent("session-a"))
        picker.on_option_list_option_selected(event)
        picker.on_option_list_option_selected(event)

        assert len(messages) == 1
        assert isinstance(messages[0], SessionPickerApp.SessionSelected)
        assert picker._selection_pending is True
...
class TestSessionPickerPublicSessions:
    ...
    def test_session_rows_do_not_include_harness_provenance_badges(self) -> None:
        session = public_session("public-session-id", 1_000)
        picker = SessionPickerApp(
            sessions=[session], latest_messages={session.id: "Public session message"}
        )

        assert _session_harness_tag(session) is None
        assert "[" not in picker._normal_option_text(session).plain
