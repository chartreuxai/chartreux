...
def test_bare_absolute_image_path_gets_at_prefix(tmp_path: Path) -> None:
    ...
    rewritten = maybe_prepend_at_for_path(str(img))
...
def test_quoted_image_path_with_spaces_is_unwrapped_and_quoted(tmp_path: Path) -> None:
    ...
    rewritten = maybe_prepend_at_for_path(pasted)
...
def test_non_image_file_path_gets_at_prefix(tmp_path: Path) -> None:
    txt = tmp_path / "readme.md"
    txt.write_text("hi")

    rewritten = maybe_prepend_at_for_path(str(txt))

    assert rewritten == f"@{txt}"


def test_missing_image_path_is_left_untouched(tmp_path: Path) -> None:
    missing = tmp_path / "nope.png"

    rewritten = maybe_prepend_at_for_path(str(missing))
...
def test_multiline_path_list_gets_at_prefixes(tmp_path: Path) -> None:
    ...
    assert maybe_prepend_at_for_path(pasted) == f"@{first} @{second}"
...
def test_relative_path_is_left_untouched(tmp_path: Path, monkeypatch) -> None:
    ...
    assert maybe_prepend_at_for_path("shot.png") == "shot.png"
...
async def test_paste_event_inserts_at_prefixed_path_into_chat_input(
    chartreux_app: ChartreuxApp, tmp_path: Path
) -> None:
    img = tmp_path / "shot.png"
    img.write_bytes(b"\x89PNG")
...
@pytest.mark.asyncio
async def test_paste_event_inserts_at_prefixed_non_image_path_into_chat_input(
    chartreux_app: ChartreuxApp, tmp_path: Path
) -> None:
...
async def test_paste_during_app_blur_forwards_to_chat_input(
    ...
    async with chartreux_app.run_test() as pilot:
        ...
        # input widget.
        chartreux_app.post_message(events.AppBlur())
        await pilot.pause()
        assert chartreux_app.focused is None

        chartreux_app.post_message(events.Paste(text=str(img)))
        await pilot.pause()

        chartreux_app.post_message(events.AppFocus())
        await pilot.pause()
...
async def test_paste_during_app_blur_forwards_non_image_path_untouched(
    ...
    async with chartreux_app.run_test() as pilot:
        ...
        chartreux_app.post_message(events.Paste(text=str(txt)))
        await pilot.pause()
...
async def test_paste_during_app_blur_not_forwarded_when_modal_screen_open(
    ...
    async with chartreux_app.run_test() as pilot:
        ...
        chartreux_app.push_screen(_BlankModal())
        await pilot.pause()
        assert chartreux_app.screen.is_modal
...
async def test_normal_paste_with_focused_widget_not_intercepted(
    ...
    async with chartreux_app.run_test() as pilot:
        ...
        text_area.focus()
        await pilot.pause()
        assert chartreux_app.focused is not None
