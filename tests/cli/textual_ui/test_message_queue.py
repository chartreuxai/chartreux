...
def _queue_controller(
    *,
    ...
    mount_and_scroll: Callable[..., Awaitable[None]] = _noop_async,
) -> QueueController:
    ...
    return QueueController(
        QueuePorts(
            mount_and_scroll=mount_and_scroll,
...
@pytest.mark.asyncio
async def test_pending_enqueue_counts_before_server_queue_exists() -> None:
    mount_started = asyncio.Event()
    release_mount = asyncio.Event()

    async def mount_and_scroll(*_args, **_kwargs) -> None:
        mount_started.set()
        await release_mount.wait()

    controller = _queue_controller(mount_and_scroll=mount_and_scroll)
    task = asyncio.create_task(controller.enqueue_prompt("queued"))
    await mount_started.wait()

    # The prompt has not reached the app server yet, but it must still block
    # incomplete-stream retry selection while the UI mount is in flight.
    assert controller.has_server_work

    release_mount.set()
    await task
    assert controller._pending_enqueues == 0
    assert controller.has_server_work

    await controller.turn_started("queue-queued")
    assert not controller.has_server_work


@pytest.mark.asyncio
async def test_pending_enqueue_clears_after_enqueue_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(UserMessage, "remove", _noop_async)

    async def failing_enqueue(_content: str, **_kwargs) -> PublicQueuedTurn:
        raise RuntimeError("enqueue failed")

    controller = _queue_controller(enqueue_turn=failing_enqueue)

    with pytest.raises(RuntimeError, match="enqueue failed"):
        await controller.enqueue_prompt("failed")
...
@pytest.mark.asyncio
async def test_pending_enqueue_counts_concurrent_calls_and_cleans_up_cancellation() -> (
    None
):
    ...
    controller = _queue_controller(mount_and_scroll=mount_and_scroll)
    first = asyncio.create_task(controller.enqueue_prompt("first"))
    await first_mount_started.wait()

    second = asyncio.create_task(controller.enqueue_prompt("second"))
    for _ in range(100):
        if controller._pending_enqueues == 2:
            break
        await asyncio.sleep(0)
    assert controller._pending_enqueues == 2
    assert controller.has_server_work

    second.cancel()
    with pytest.raises(asyncio.CancelledError):
        await second
    assert controller._pending_enqueues == 1
    assert controller.has_server_work
...
@pytest.mark.asyncio
async def test_enqueue_reservation_is_nesting_safe() -> None:
    controller = _queue_controller()

    with controller.reserve_enqueue():
        assert controller._pending_enqueues == 1
        assert controller.has_server_work
        with controller.reserve_enqueue():
            assert controller._pending_enqueues == 2
...
@pytest.mark.asyncio
async def test_enqueue_prompt_uses_prepared_prompt() -> None:
...
@pytest.mark.asyncio
async def test_turn_started_unpends_block() -> None:
...
@pytest.mark.asyncio
async def test_steer_pending_preserves_block_when_reenqueue_also_fails(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(UserMessage, "remove", _noop_async)
    enqueue_calls = {"n": 0, "contents": []}

    async def enqueue_turn(content: str, **kwargs) -> PublicQueuedTurn:
        enqueue_calls["n"] += 1
        enqueue_calls["contents"].append(content)
        if enqueue_calls["n"] == 2:
            raise RuntimeError("enqueue boom")
        return _server_turn(
            f"item-{enqueue_calls['n']}",
            content,
            message_entry_id=kwargs.get("message_entry_id"),
        )
    ...
    # Steer fails, then re-enqueue also fails: retain the unresolved prompt
    ...
    # The retained local-only prompt remains editable. A later submission
    # appends to it and restores both prompts as one queued server item.
    assert await controller.update_prompt(0, "edited")
    await controller.enqueue_prompt("next")

    assert [widget.get_content() for widget in controller.widgets] == ["edited", "next"]
