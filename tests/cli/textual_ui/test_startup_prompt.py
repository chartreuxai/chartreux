...
async def test_startup_prompt_waits_for_startup_resume_picker(
    ...
    app = build_test_chartreux_app(initial_prompt="continue the work")
    await app.prepare()
...
@pytest.mark.parametrize("theme", [AUTO_THEME, "dark"])
def test_run_textual_ui_warms_auto_theme_without_editor_promo_read(
    theme: str, tmp_path
) -> None:
    ...
    with (
        patch("chartreux.cli.textual_ui.app.resolve_auto_theme") as resolve_auto_theme,
        patch("chartreux.cli.textual_ui.app.ChartreuxApp") as chartreux_app,
        patch(
            "chartreux.utils.cache_store.FileSystemCacheStore.read_section",
            side_effect=AssertionError("startup must not read the editor promo cache"),
        ),
        patch(
            "chartreux.cli.textual_ui.app._run_app_with_cleanup",
        ...
        run_textual_ui(
            start_app_server=start_app_server, history_file=tmp_path / "history"
        )

    resolve_auto_theme.assert_called_once_with()
    chartreux_app.assert_called_once()
    assert "vscode_extension_promo" not in chartreux_app.call_args.kwargs
