...
@pytest.mark.asyncio
async def test_numeric_input_is_not_reserved_for_removed_feedback_ui() -> None:
    app = build_test_chartreux_app()
    async with app.run_test() as pilot:
        ...
        assert not hasattr(app.app_server.resources, "feedback")
        assert not hasattr(app, "_feedback_bar")

        await pilot.press("1", "0", "2", "3")
        await pilot.pause()
