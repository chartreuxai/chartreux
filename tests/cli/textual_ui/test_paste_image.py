...
from chartreux.cli.textual_ui.widgets.chat_input import paste_image
...
def _build_vision_app(*, supports_images: bool = True) -> ChartreuxApp:
    ...
    return build_test_chartreux_app(config=config)
...
async def _press_ctrl_v_under_platform(monkeypatch, system: str) -> tuple[bool, list]:
    ...
    text_area_module = importlib.import_module(
        "chartreux.cli.textual_ui.widgets.chat_input.text_area"
    )
...
async def test_empty_paste_posts_clipboard_image_pasted_message(
    ...
    async with chartreux_app.run_test() as pilot:
        text_area = chartreux_app.query_one(ChatTextArea)
        text_area.focus()
        original_post = text_area.post_message
...
async def test_handle_clipboard_image_paste_writes_file_and_inserts_token() -> None:
    ...
    with (
        patch(
            "chartreux.cli.textual_ui.widgets.chat_input.paste_image.read_clipboard_image",
            return_value=_FAKE_PNG,
        ),
        patch(
            "chartreux.cli.textual_ui.widgets.chat_input.paste_image.is_clipboard_image_paste_supported",
            return_value=True,
        ),
    ):
...
async def test_handle_clipboard_image_paste_rejects_oversize(monkeypatch) -> None:
    ...
    notifications: list[str] = []
    monkeypatch.setattr(
        "chartreux.cli.textual_ui.widgets.chat_input.paste_image.MAX_IMAGE_BYTES", 10
    )
    with (
        patch(
            "chartreux.cli.textual_ui.widgets.chat_input.paste_image.read_clipboard_image",
