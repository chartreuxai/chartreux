...
async def test_resume_picker_opens_one_fresh_session() -> None:
    ...
    # selection to AppChartreux's in-app picker: it must not resume here, and must not
...
async def test_continue_opens_fresh_session_with_auto_continue_descriptor() -> None:
    ...
    # and sets continue_latest so AppChartreux drives the resume in-app.
    host = MagicMock()
    host.cwd = "/workspace"
    session = MagicMock()
    host.start_session = AsyncMock(return_value=session)
...
async def test_resume_with_id_opens_fresh_session_with_auto_resume_descriptor() -> None:
    ...
    # open_session) and sets resume_session_id so AppChartreux resumes in-app.
    host = MagicMock()
    host.cwd = "/workspace"
    session = MagicMock()
    host.start_session = AsyncMock(return_value=session)
