...
from chartreux.core.skills.models import SkillInfo, SkillMetadata


def test_registry_metadata_is_not_a_local_skill_contract() -> None:
    assert "registry" not in SkillInfo.model_fields
    with pytest.raises(ValidationError, match="source"):
        SkillInfo.model_validate({
            "name": "test-skill",
...
class TestSkillInfo:
    ...
    def test_can_omit_skill_path_for_builtin_inline_prompt(self) -> None:
        info = SkillInfo(
            name="chartreux", description="Builtin skill", prompt="Inline prompt"
        )
