...
@pytest.fixture
def skill_manager(vibe_config: ChartreuxConfigSchema) -> SkillManager:
...
class TestSkillManagerSearchPaths:
    def test_discovers_from_vibe_skills_when_cwd_trusted(
        ...
        vibe_skills = tmp_working_directory / ".chartreux" / "skills"
        vibe_skills.mkdir(parents=True)
        create_skill(vibe_skills, "vibe-skill", "Skill from .chartreux/skills")
    ...
    def test_discovers_from_both_vibe_and_agents_skills_when_cwd_trusted(
        ...
        vibe_skills.mkdir(parents=True)
        agents_skills.mkdir(parents=True)
        create_skill(vibe_skills, "vibe-only", "From .chartreux")
    ...
    def test_first_discovered_wins_when_same_skill_in_vibe_and_agents(
        ...
        vibe_skills.mkdir(parents=True)
        agents_skills.mkdir(parents=True)
        create_skill(vibe_skills, "shared-skill", "First from .chartreux")
...
class TestParseSkillCommand:
    ...
    def test_parses_skill_without_args(
        self, skills_dir: Path, skill_config: ChartreuxConfigSchema
    ) -> None:
