...
from chartreux.core.llm_models import ImageAttachment, InlineImageSource
...
def test_agents_update_carries_evictions() -> None:
    ...
    event = projection.consume(
        Notification(
            method="agents/update", params=params.model_dump(mode="json", by_alias=True)
        )
    )
...
def test_agents_update_accepts_legacy_payload_without_evictions() -> None:
    projection = _projection()
    params = {
        "eventId": 1,
        "sessionId": "session-1",
        "emittedAt": 2,
        "agents": [{"agentId": "agent-1", "profile": "worker", "availability": "idle"}],
    }

    event = projection.consume(Notification(method="agents/update", params=params))

    assert isinstance(event, AgentsUpdate)
    assert event == AgentsUpdate(event.agents, [])
