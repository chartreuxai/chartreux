...
from chartreux.core.session import worktrees as session_worktrees
...
@pytest.mark.asyncio
async def test_a_worktree_keeps_explicitly_authorized_extra_roots(
    tmp_path: Path,
) -> None:
    _init_repo(tmp_path)
    attachments = tmp_path.parent / "attachments"
    attachments.mkdir()
    attachment_alias = tmp_path / "attachment-alias"
    attachment_alias.symlink_to(attachments, target_is_directory=True)
    connection = await connect_backend_contract_host(
        session_options=SessionOptions(
            cwd=str(tmp_path),
            workspace_roots=[str(tmp_path), str(attachments), str(attachment_alias)],
            worktree=NewWorktreeInput(branch="jun/attachments", name="attachments"),
        ),
        capabilities=ClientCapabilities(),
    )
    try:
        ...
        assert session.state.session.workspace_roots == [
            str(cwd.resolve()),
            str(attachments.resolve()),
        ]
        assert (
            str(tmp_path.parent.resolve()) not in session.state.session.workspace_roots
        )
    finally:
        await connection.host.close()
...
async def test_attaching_a_session_sweeps_the_repository_for_abandoned_worktrees(
    ...
    def record(
        ...
        in_use = tuple(resumable_snapshot_provider())
        loop.call_soon_threadsafe(swept.put_nowait, (base, in_use))

    monkeypatch.setattr(session_worktrees, "sweep_claims", record)
