...
from chartreux.user_content import UserResourceLink
...
async def test_read_file_tool_call_runs_through_the_backend(
    ...
    tool_arguments = {"path": str(target)} if False else {"file_path": str(target)}
    backend_contract_mistral_api.mock(
        side_effect=[
    ...
    connection = await connect_backend_contract_host(
        session_options=SessionOptions(cwd=str(tmp_path), enabled_tools=["read_file"]),
...
@pytest.mark.asyncio
async def test_shell_tool_call_runs_without_approval(
    backend_contract_mistral_api: respx.Route,
    backend_contract_mistral_response: Callable[..., httpx.Response],
    tmp_path,
) -> None:
    ...
    try:
        ...
        async for event in session.act("run shell"):
...
@pytest.mark.asyncio
async def test_interrupt_closes_user_input_callback(
    backend_contract_mistral_api: respx.Route,
    backend_contract_mistral_response: Callable[..., httpx.Response],
    tmp_path,
) -> None:
    ...
    connection = await connect_backend_contract_host(
        session_options=SessionOptions(
            cwd=str(tmp_path), enabled_tools=["ask_user_question"]
        ),
        capabilities=ClientCapabilities(callback_kinds=["user_input"]),
    )
