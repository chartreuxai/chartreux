...
from chartreux.core.tools.builtins.read_image import ReadImage, ReadImageResult
from chartreux.core.tools.builtins.skill import Skill, SkillArgs
from chartreux.core.tools.builtins.web_search import WebSearch, WebSearchResult
from chartreux.core.tools.search import SearchSource
...
def test_shell_projection_keeps_the_transcript_a_tool_streamed_itself() -> None:
    event = ToolResultEvent(
        tool_name="bash",
        tool_class=Bash,
        result=CapturedShellResult(command="pwd", stdout="one two"),
        tool_call_id="call-1",
    )
    event = event.model_copy(
        update={"presentation": ToolUIDataAdapter(Bash).get_result_presentation(event)}
    )
...
def _project_shell_result(
    tool_class, result: CapturedShellResult
) -> CompletedEffectState:
    event = ToolResultEvent(
...
@pytest.mark.parametrize("tool_class", [Bash])
def test_bash_projects_both_captured_streams(tool_class) -> None:
...
def test_web_search_projection_preserves_non_mistral_sources() -> None:
    result = WebSearchResult(
        ...
        sources=[
            SearchSource(
                title="Python docs", url="https://docs.python.org", snippet="Async I/O"
            )
        ],
        was_truncated=False,
    )
    ...
    event = event.model_copy(
        update={
            "presentation": ToolUIDataAdapter(WebSearch).get_result_presentation(event)
        }
    )
...
def test_historical_web_search_output_without_new_fields_validates() -> None:
    output = WebSearchEffectOutput.model_validate({
        "query": "python async",
        "answer": "Use asyncio",
    ...
    assert output.was_truncated is None
    assert output.sources[0].snippet is None


def test_read_image_projection_keeps_summary_without_file_content() -> None:
    result = ReadImageResult(
        file_path="image.png", alias="image.png", mime_type="image/png", size_bytes=42
    )
    event = ToolResultEvent(
        tool_name="read_image",
        tool_class=ReadImage,
        result=result,
        tool_call_id="call-1",
    )
    event = event.model_copy(
        update={
            "presentation": ToolUIDataAdapter(ReadImage).get_result_presentation(event)
        }
    )
    ...
    assert completed.display.message == "image image.png"
