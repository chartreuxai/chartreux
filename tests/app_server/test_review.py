...
from chartreux.core.checkpoints import AgentTurn, FileSnapshot, FileState
...
@pytest.mark.parametrize("operation", ["approve", "revert"])
@pytest.mark.asyncio
async def test_review_unknown_file_is_invalid_params_and_preserved(
    tmp_path: Path, operation: str
) -> None:
    path = tmp_path / "untracked.bin"
    original = b"must remain\x00unchanged"
    path.write_bytes(original)
    loop = build_test_agent_loop()
    session = await create_test_app_server_session(loop)
    try:
        with pytest.raises(AppServerResponseError) as error:
            await getattr(session.resources.review, operation)(
                ReviewFileTarget(path=str(path.resolve()))
            )
    finally:
        await session.close()

    assert error.value.error.code is ProtocolErrorCode.INVALID_PARAMS
    assert "no checkpoint" in error.value.error.message
