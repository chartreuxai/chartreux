...
def _responder(failures: list[Any]) -> Any:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if isinstance(failure, int):
            body = (
                {"error": {"code": "rate_limit_exceeded", "message": "nope"}}
                if failure == 429
                else {"message": "nope"}
            )
            return httpx.Response(failure, json=body)
...
@pytest.mark.parametrize("failure", [400, 401, 404, 422])
@pytest.mark.asyncio
...
def _vibe_client(transport: httpx.MockTransport) -> Any:
    ...
    return ChartreuxAsyncHTTPClient(transport=transport)
...
def _patch_client_transport(
    ...
    under test.
    """
    from chartreux.utils import http as http_module

    original = http_module.ChartreuxAsyncHTTPClient
    ...
    monkeypatch.setattr(
        "chartreux.core.llm.backend.mistral.ChartreuxAsyncHTTPClient", factory
    )
