...
from chartreux.core.session import image_snapshot
...
def test_snapshot_rejects_existing_partial_file_after_rename_race(
    ...
    digest = hashlib.sha1(PNG_BYTES, usedforsecurity=False).hexdigest()
    dest = attachments / f"{digest}.png"

    def race(*_args: object) -> None:
        dest.write_bytes(b"partial")
        raise FileExistsError

    monkeypatch.setattr(image_snapshot.os, "replace", race)

    with pytest.raises(ImageSnapshotError, match="Existing image snapshot is invalid"):
        image_snapshot._persist(PNG_BYTES, ext=".png", session_dir=session_dir)

    assert list(attachments.glob(f".{digest}*")) == []
...
def test_snapshot_image_bytes_writes_file_when_session_dir_is_set(
    ...
    assert stat.S_IMODE(att.source.path.stat().st_mode) == 0o600
