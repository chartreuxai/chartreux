...
from chartreux.core.review import (
    AllTarget,
    FileTarget,
    LastTurnsTarget,
    OpaqueReviewRegion,
...
class TestReviewErrors:
    @pytest.mark.parametrize("operation", ["approve_review", "revert_review"])
    def test_unknown_file_target_is_rejected_without_mutation(
        self, tmp_path: Path, operation: str
    ) -> None:
        sh = _shells(_make_messages("m"))
        known = tmp_path / "known.txt"
        unknown = tmp_path / "unknown.txt"
        resolved_known = self._tracked_change(sh, known)
        unknown.write_bytes(b"unchanged\x00bytes")
        events_before = list(sh.recorder._checkpointer._events)

        with pytest.raises(ReviewError, match="has no checkpoint"):
            getattr(sh.review, operation)(FileTarget(path=str(unknown.resolve())))

        assert unknown.read_bytes() == b"unchanged\x00bytes"
        assert sh.recorder._checkpointer._events == events_before
        assert resolved_known in sh.recorder._checkpointer.view().tracked_paths

    def test_regions_preflight_rejects_entire_target_before_reconciliation(
        self, tmp_path: Path
    ) -> None:
        sh = _shells(_make_messages("m"))
        known = tmp_path / "known.txt"
        path = self._tracked_change(sh, known)
        region = sh.review.review_state().files[0].regions[0]
        known.write_text("manual drift\n", encoding="utf-8")
        events_before = list(sh.recorder._checkpointer._events)

        with pytest.raises(ReviewError, match="unknown region"):
            sh.review.approve_review(
                RegionsTarget(
                    path=path,
                    regions=((region.version_index, region.ordinal), (999999, 0)),
                )
            )

        assert known.read_text(encoding="utf-8") == "manual drift\n"
        assert sh.recorder._checkpointer._events == events_before

    def test_tracked_file_without_pending_changes_is_valid_target(
        self, tmp_path: Path
    ) -> None:
        sh = _shells(_make_messages("m"))
        ...
        assert sh.review.approve_review(FileTarget(path=str(f.resolve()))) == [
            str(f.resolve())
        ]
        assert f.read_text(encoding="utf-8") == "same\n"

    @staticmethod
    def _tracked_change(sh: _Shells, path: Path) -> str:
