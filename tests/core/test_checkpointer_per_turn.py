...
class TestAttribution:
    def test_has_checkpoint_distinguishes_unknown_from_tracked_noop(self) -> None:
        cp = Checkpointer()
        cp.begin_turn(1)
        cp.record_pre_edit("known", _txt("same\n"))
        cp.record_post_edit("known", _txt("same\n"))
        cp.seal_turn()

        assert cp.has_checkpoint("known")
        assert not cp.has_checkpoint("unknown")
        assert cp.view().hunk_order("known") == []
