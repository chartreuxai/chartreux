...
class TestBudgetBoundedRetry:
    ...
    async def test_deadline_is_rechecked_after_retry_observer(
        self, fake_clock: list[float], generator: bool
    ) -> None:
        attempts = 0

        async def consume_deadline(_reason: RetryReason) -> None:
            fake_clock[0] = 2.0
        ...
        if generator:
            ...
            @async_generator_retry(
                ...
                delay_seconds=0.0,
                on_retry=consume_deadline,
            )
            async def generator_operation() -> AsyncGenerator[str]:
                nonlocal attempts
                attempts += 1
                raise httpx.ConnectError("down", request=_make_request())
            ...
            with pytest.raises(httpx.ConnectError):
                _ = [item async for item in generator_operation()]
        else:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("generator", [False, True])
    async def test_deadline_is_rechecked_after_backoff(
        self, fake_clock: list[float], generator: bool
    ) -> None:
        attempts = 0
        ...
        if generator:
            ...
            @async_generator_retry(tries=3, max_elapsed_time=1.0, delay_seconds=1.0)
            async def generator_operation() -> AsyncGenerator[str]:
                nonlocal attempts
                attempts += 1
                raise httpx.ConnectError("down", request=_make_request())
            ...
            @async_retry(tries=3, max_elapsed_time=1.0, delay_seconds=1.0)
            async def operation() -> str:
                nonlocal attempts
                attempts += 1
                raise httpx.ConnectError("down", request=_make_request())
...
class TestParseRetryAfter:
    def test_delta_seconds(self) -> None:
        error = _make_http_status_error(503, {"Retry-After": "7"})
        assert _parse_retry_after(error) == 7.0

    def test_delta_seconds_with_surrounding_whitespace(self) -> None:
        error = _make_http_status_error(503, {"Retry-After": "  12 "})
    ...
    def test_empty_header_returns_none(self) -> None:
        assert (
            _parse_retry_after(_make_http_status_error(503, {"Retry-After": ""}))
            is None
        )

    def test_unparsable_header_returns_none(self) -> None:
        error = _make_http_status_error(503, {"Retry-After": "soon"})
...
class TestAdaptiveBackoff:
    ...
    async def test_async_retry_honors_retry_after_over_exponential(
        ...
        async def call() -> str:
            ...
            attempts += 1
            if attempts < 3:
                raise _make_http_status_error(503, {"Retry-After": "5"})
            return "ok"
    ...
    async def test_async_retry_caps_retry_after(self, sleeps: list[float]) -> None:
        ...
        async def call() -> str:
            raise _make_http_status_error(503, {"Retry-After": "3600"})
