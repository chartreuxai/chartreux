...
from chartreux.core.utils.merge import MergeConflictError, MergeKeyError, MergeStrategy
...
class TestConcat:
    ...
    def test_empty_mapping_override_is_rejected(self) -> None:
        with pytest.raises(TypeError, match="CONCAT requires list operands"):
            MergeStrategy.CONCAT.apply([1, 2], {})

    def test_empty_mapping_base_is_rejected(self) -> None:
        with pytest.raises(TypeError, match="CONCAT requires list operands"):
            MergeStrategy.CONCAT.apply({}, [1, 2])

    def test_both_empty_mappings_are_rejected(self) -> None:
        with pytest.raises(TypeError, match="CONCAT requires list operands"):
...
class TestUnion:
    ...
    def test_empty_mapping_override_is_rejected(self) -> None:
        with pytest.raises(TypeError, match="UNION requires list operands"):
            MergeStrategy.UNION.apply([1], {}, key_fn=str)
...
class TestDeepMerge:
    def test_dicts_merged_recursively(self) -> None:
        ...
        override = {
            "bash": {"command_patterns": ["git status"], "metadata": {"timeout": 60}}
        }
