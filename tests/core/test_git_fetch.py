...
import chartreux.core.git.fetch as fetch_module
from chartreux.core.git.fetch import (
    UnsafeGitFetchError,
    fetch_remote,
    prepare_secure_fetch,
)
...
@pytest.mark.parametrize(
    "include_kind", ["direct", "symlink", "transitive", "includeIf"]
)
@pytest.mark.parametrize(
    "planted_content",
    [
        "[credential]\n\thelper = planted\n",
        '[url "ssh://planted.invalid/"]\n\tinsteadOf = https://example.invalid/\n',
    ],
)
def test_global_include_from_checkout_is_not_replayed(
    tmp_path: Path,
    ...
    planted = checkout / "planted.gitconfig"
    planted.write_text(planted_content)
    source = planted
    if include_kind == "symlink":
        source = tmp_path / "home" / "link.gitconfig"
        source.parent.mkdir()
        source.symlink_to(planted)
    if include_kind == "transitive":
        source = tmp_path / "intermediate.gitconfig"
        source.write_text(f"[include]\n\tpath = {planted}\n")
    if include_kind == "includeIf":
        contents = f'[includeIf "gitdir:{checkout}/"]\n\tpath = {source}\n'
    else:
        contents = f"[include]\n\tpath = {source}\n"
    _write_test_global_config(tmp_path, monkeypatch, contents)

    with pytest.raises(UnsafeGitFetchError, match="repository is not trusted"):
        prepare_secure_fetch(repo, "origin")


def test_git_metadata_outside_checkout_is_not_trusted(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    checkout = tmp_path / "checkout"
    git_dir = tmp_path / "git-metadata"
    repo = Repo.init(checkout, separate_git_dir=git_dir, allow_unsafe_options=True)
    repo.config_writer().set_value(
        'remote "origin"', "url", "https://example.invalid/repo.git"
    ).release()
    planted = git_dir / "planted.gitconfig"
    planted.write_text("[credential]\n\thelper = planted\n")
    _write_test_global_config(tmp_path, monkeypatch, f"[include]\n\tpath = {planted}\n")

    with pytest.raises(UnsafeGitFetchError, match="repository is not trusted"):
        prepare_secure_fetch(repo, "origin")


def test_checkout_symlink_to_external_config_is_not_trusted(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    repo = _repo_with_remote(tmp_path / "checkout", "https://example.invalid/repo.git")
    external = tmp_path / "external.gitconfig"
    external.write_text("[credential]\n\thelper = planted\n")
    planted_link = tmp_path / "checkout" / "planted.gitconfig"
    planted_link.symlink_to(external)
    _write_test_global_config(
        tmp_path, monkeypatch, f"[include]\n\tpath = {planted_link}\n"
    )
    with pytest.raises(UnsafeGitFetchError, match="repository is not trusted"):
        prepare_secure_fetch(repo, "origin")


def test_global_include_intermediate_symlink_through_checkout_rejected(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    repo = _repo_with_remote(tmp_path / "checkout", "https://example.invalid/repo.git")
    external = tmp_path / "outside.gitconfig"
    external.write_text("[credential]\n\thelper = planted\n")
    inside = tmp_path / "checkout" / "link.gitconfig"
    inside.symlink_to(external)
    home = tmp_path / "home"
    home.mkdir()
    outer = home / "include.gitconfig"
    outer.symlink_to(inside)
    _write_test_global_config(tmp_path, monkeypatch, f"[include]\n\tpath = {outer}\n")
    with pytest.raises(UnsafeGitFetchError, match="repository is not trusted"):
        prepare_secure_fetch(repo, "origin")


def test_global_include_outside_checkout_is_trusted(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    repo = _repo_with_remote(tmp_path / "checkout", "https://example.invalid/repo.git")
    assert (
        _config_from_env(prepare_secure_fetch(repo, "origin").env)["credential.helper"]
        == "trusted"
    )


@pytest.mark.parametrize("origin", ["command line:", "file:relative", "file:/missing"])
def test_unclassifiable_fetch_config_origin_fails_closed(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, origin: str
) -> None:
    repo = _repo_with_remote(tmp_path / "checkout", "https://example.invalid/repo.git")
    original_run = fetch_module.subprocess.run

    def fake_run(command: list[str], **kwargs: Any) -> subprocess.CompletedProcess[str]:
        if "--show-origin" in command:
            return subprocess.CompletedProcess(
                command, 0, f"global\0{origin}\0credential.helper\nplanted\0", ""
            )
        return original_run(command, **kwargs)

    monkeypatch.setattr(fetch_module.subprocess, "run", fake_run)
    with pytest.raises(UnsafeGitFetchError, match="classify"):
        prepare_secure_fetch(repo, "origin")
...
@pytest.mark.parametrize("variable", ["HOME", "XDG_CONFIG_HOME"])
...
def test_project_and_relative_path_ssh_is_not_used(
    ...
    planted = _write_executable(project / "ssh")
    trusted = _write_executable(trusted_bin / "ssh")
...
def test_https_fetch_prepares_ssh_for_trusted_global_url_rewrite(
    ...
    project = tmp_path / "checkout"
    trusted = _write_executable(tmp_path / "trusted-bin" / "ssh")
...
def test_unresolvable_project_directory_fails_closed(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    gone = tmp_path / "gone"
    gone.mkdir()
    ...
    with pytest.raises(UnsafeGitFetchError, match="Cannot resolve the repository"):
        fetch_module._resolved_project_dir(cast(Repo, _UnresolvableRepo()))
