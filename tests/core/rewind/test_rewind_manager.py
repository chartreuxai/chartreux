...
from chartreux.core.compaction import select_model_context
from chartreux.core.llm_models import LLMMessage, Role
from chartreux.core.message_list import MessageList
from chartreux.core.rewind import RewindError, RewindManager
...
class TestRewind:
    ...
    def test_get_rewindable_messages_excludes_injected(self) -> None:
        ...
        messages.append(
            LLMMessage(role=Role.user, content="middleware reminder", injected=True)
        )
    ...
    async def test_rewind_after_open_turn_reset_does_not_block_accepted_frontier(
        ...
        from chartreux.core.checkpoints.models import AgentTurn
        from chartreux.core.review import ReviewManager, ScopeTarget
...
class TestRewindScenarios:
    ...
    async def test_rewind_restores_errors_collected(self, tmp_path: Path) -> None:
        ...
        turn1_idx = sh.rewind.get_rewindable_messages()[0][0]
        with patch(
            "chartreux.core.checkpoints.fs.os.remove",
