...
from chartreux.core.config.schema import ConfigSchema, WithConcatMerge, WithReplaceMerge
...
async def test_agent_profile_layer_replaces_full_payload() -> None:
    ...
    await layer.apply(
        ConfigPatch(
            ReplaceOperationPatch(path="", value={"disabled_tools": ["write_file"]}),
            fingerprint=fingerprint,
            reason="agent profile changed",
        )
    )

    assert (await layer.load()).model_dump() == {"disabled_tools": ["write_file"]}


@pytest.mark.asyncio
async def test_agent_profile_layer_clear_removes_profile_overrides() -> None:
    layer = AgentProfileLayer(data={"disabled_tools": ["write_file"]})
...
class FakeLocalUserLayer(ConfigLayer[UserConfigSchema]):
    """Simulates ~/.chartreux/config.toml — always trusted, typed output."""
...
def test_toml_snapshot_drops_none_optional_fields_and_round_trips(
    ...
    from chartreux.core.config.layers._base import (
        _read_toml_snapshot,
