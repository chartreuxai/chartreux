...
from chartreux.core.config.layers import project as project_layer
...
async def test_re_rooting_moves_which_project_config_is_read(
    ...
    trusted_folders_manager.add_trusted(tmp_working_directory / ".chartreux")
    trusted_folders_manager.add_trusted(destination / ".chartreux")
...
async def test_re_rooting_moves_where_project_config_is_written(
    ...
    layer = orchestrator.get_layer(ProjectConfigLayer.NAME)
    assert isinstance(layer, ProjectConfigLayer)
    assert layer.config_file_path == destination / ".chartreux" / "config.toml"
    ...
    assert failures == []
    with (destination / ".chartreux" / "config.toml").open("rb") as f:
        assert tomllib.load(f)["theme"] == "moved-theme"
    with (tmp_working_directory / ".chartreux" / "config.toml").open("rb") as f:
...
async def test_an_untrusted_destination_contributes_nothing(
    ...
    trusted_folders_manager.trust_for_session(destination / ".chartreux")
    await _reroot(orchestrator, destination)
    await orchestrator.reload()
...
async def test_a_find_already_running_cannot_undo_the_move(
    ...
    _project_config(tmp_working_directory, {"theme": "origin-theme"})
    _project_config(destination, {"theme": "destination-theme"})
    trusted_folders_manager.trust_for_session(tmp_working_directory / ".chartreux")
    trusted_folders_manager.trust_for_session(destination / ".chartreux")
