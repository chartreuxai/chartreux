...
def test_ensure_parent_paths_vivifies_missing_dicts() -> None:
    data: dict[str, Any] = {}
    ops = [AddOperationPatch(path="/tools/bash/command_patterns", value=["ls"])]
...
def test_ensure_parent_paths_leaves_existing_dicts() -> None:
    data: dict[str, Any] = {"tools": {"bash": {"command_patterns": ["ls"]}}}
    ops = [AddOperationPatch(path="/tools/bash/command_patterns", value=["cat"])]

    result = ensure_parent_paths(data, ops)

    assert result == {"tools": {"bash": {"command_patterns": ["ls"]}}}
...
def test_ensure_parent_paths_ignores_non_add_ops() -> None:
    data: dict[str, Any] = {}
    ops: list[PatchOp] = [
        ReplaceOperationPatch(path="/tools/bash/command_patterns", value=["ls"]),
        RemoveOperationPatch(path="/tools/bash/command_patterns"),
    ]
