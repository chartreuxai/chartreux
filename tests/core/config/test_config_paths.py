...
class TestCwdIsUserConfigHome:
    def test_true_when_cwd_is_user_home(self) -> None:
        mgr = HarnessFilesManager(
            sources=("user", "project"), cwd=CHARTREUX_HOME.path.parent
        )
    ...
    def test_hook_files_not_duplicated_when_cwd_is_home(
        ...
        assert mgr.hook_files == [CHARTREUX_HOME.path / "hooks.toml"]
    ...
    def test_config_file_is_user_config_when_cwd_is_home(
        ...
        assert mgr.config_file == CHARTREUX_HOME.path / "config.toml"
...
class TestProjectToolsDirs:
    ...
    def test_returns_path_when_tools_dir_exists_and_trusted(
        ...
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        tools_dir = tmp_path / ".chartreux" / "tools"
    ...
    def test_ignores_tools_when_file_not_dir(
        ...
        (tmp_path / ".chartreux" / "tools").write_text("", encoding="utf-8")
        mgr = HarnessFilesManager(sources=("user", "project"))
        assert mgr.project_tools_dirs == []
    ...
    def test_does_not_find_nested_tools_dirs(
        ...
        mgr = HarnessFilesManager(sources=("user", "project"))
        assert mgr.project_tools_dirs == [tmp_path / ".chartreux" / "tools"]
...
class TestProjectAgentsDirs:
    ...
    def test_returns_path_when_agents_dir_exists_and_trusted(
        ...
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        agents_dir = tmp_path / ".chartreux" / "agents"
    ...
    def test_ignores_agents_when_file_not_dir(
        ...
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        (tmp_path / ".chartreux").mkdir()
        (tmp_path / ".chartreux" / "agents").write_text("", encoding="utf-8")
        mgr = HarnessFilesManager(sources=("user", "project"))
        assert mgr.project_agents_dirs == []
    ...
    def test_does_not_find_nested_agents_dirs(
        ...
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        (tmp_path / ".chartreux" / "agents").mkdir(parents=True)
        (tmp_path / "sub" / "deep" / ".chartreux" / "agents").mkdir(parents=True)
        mgr = HarnessFilesManager(sources=("user", "project"))
        assert mgr.project_agents_dirs == [tmp_path / ".chartreux" / "agents"]
...
class TestProjectSkillsDirs:
    ...
    def test_returns_vibe_skills_only_when_only_it_exists(
        ...
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        vibe_skills = tmp_path / ".chartreux" / "skills"
    ...
    def test_ignores_vibe_skills_when_file_not_dir(
        ...
        monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
        (tmp_path / ".chartreux").mkdir()
        (tmp_path / ".chartreux" / "skills").write_text("", encoding="utf-8")
        mgr = HarnessFilesManager(sources=("user", "project"))
        assert mgr.project_skills_dirs == []
    ...
    def test_does_not_find_nested_skills_dirs(
        ...
        (tmp_path / "sub" / "deep" / ".chartreux" / "skills").mkdir(parents=True)
        mgr = HarnessFilesManager(sources=("user", "project"))
        assert mgr.project_skills_dirs == [tmp_path / ".chartreux" / "skills"]
