...
class MiniChartreuxConfigSchema(ConfigSchema):
...
def test_session_store_path_preserves_resolved_symlink_denotation(
    tmp_path: Path,
) -> None:
    a = tmp_path / "a"
    b = tmp_path / "b"
    a.mkdir()
    b.mkdir()
    (a / "link").symlink_to(b, target_is_directory=True)

    configured = a / "link" / ".." / "sessions"
    config = SessionLoggingConfig(save_dir=str(configured))

    assert Path(config.save_dir) == tmp_path / "sessions"
    assert Path(config.permission_repair_dir) == configured


def test_default_session_store_preserves_literal_permission_repair_path(
    ...
    monkeypatch.setenv("CHARTREUX_HOME", str(link))

    config = SessionLoggingConfig()

    assert Path(config.save_dir) == target / "logs" / "session"
    assert Path(config.permission_repair_dir) == link / "logs" / "session"
...
def test_scenario_mini_vibe_config_schema() -> None:
    config = MiniChartreuxConfigSchema(
