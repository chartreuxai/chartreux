...
from chartreux.core.config.types import ConflictStrategy
...
async def test_apply_persists_to_in_memory_data() -> None:
    ...
    patch = ConfigPatch(
        AddOperationPatch(path="/tools/bash/permission", value="always"),
...
@pytest.mark.asyncio
async def test_accepted_staged_write_survives_force_reload() -> None:
    import copy

    layer = OverridesLayer(data={"theme": "auto"})
    await layer.load()
    staged = copy.deepcopy(layer)
    await staged.apply(
        ConfigPatch(
            ReplaceOperationPatch(path="/theme", value="dark"),
            fingerprint=staged.fingerprint or "",
        )
    )
    assert (await layer.load(force=True)).model_dump() == {"theme": "auto"}
    layer._accept_loaded_state(staged)
    assert (await layer.load(force=True)).model_dump() == {"theme": "dark"}
    await staged.apply(
        ConfigPatch(
