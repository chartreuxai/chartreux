...
from chartreux.core.hooks._handler import HookOutputError, _parse_structured_response
from chartreux.core.hooks.config import (
    HookConfigResult,
    load_hooks_file,
    load_hooks_from_fs,
)
...
from chartreux.core.hooks.models import (
    HookConfig,
    HookEndEvent,
    HookEvent,
    HookMessageSeverity,
...
@pytest.mark.parametrize("source", list(HookSource))
def test_runtime_hook_preserves_direct_sources(source: HookSource) -> None:
    config = _make_hook()
    definition = RuntimeHookDefinition(config=config, source=source)
    assert definition.config == config
    assert definition.source == source


@pytest.mark.parametrize("source", ["project_plugin", "global_plugin"])
def test_runtime_hook_rejects_removed_plugin_sources(source: str) -> None:
    with pytest.raises(ValidationError, match="source"):
        RuntimeHookDefinition.model_validate({"config": _make_hook(), "source": source})


def test_runtime_hook_rejects_removed_plugin_metadata() -> None:
    with pytest.raises(ValidationError, match="plugin_name"):
        RuntimeHookDefinition.model_validate({
            "config": _make_hook(),
            "source": "project",
            "plugin_name": "obsolete",
        })
...
def _shell_cmd(script: str) -> str:
    return f"sh -c {shlex.quote(script)}"


def _emit_cmd(payload: dict[str, Any]) -> str:
    """Build an argv command that prints ``payload`` as JSON to stdout.

    Uses Python so embedded quotes and metacharacters in field values are passed
    safely as a single argument to ``-c``.
    """
...
class TestConfigLoading:
    ...
    def test_load_from_both_global_and_project(
        ...
        project_vibe = tmp_working_directory / ".chartreux"
        _write_hooks_toml(
...
class TestHookExecutor:
    ...
    async def test_nonzero_exit_passes_through(
        ...
        hook = _make_hook(command=_shell_cmd("echo 'oops'; exit 1"))
        result = await HookExecutor().run(hook, sample_invocation)
    ...
    async def test_stderr_captured_separately(
        ...
        hook = _make_hook(command=_shell_cmd("echo out; echo err >&2"))
        result = await HookExecutor().run(hook, sample_invocation)
        assert result.exit_code == 0
    ...
    @pytest.mark.asyncio
    async def test_quoted_executable_path_with_spaces(
        self, sample_invocation: PostAgentInvocation, tmp_path: Path
    ) -> None:
        executable = tmp_path / "python with spaces"
        executable.symlink_to(sys.executable)
        command = f"{shlex.quote(str(executable))} -c 'print(\"quoted path\")'"

        result = await HookExecutor().run(
            _make_hook(command=command), sample_invocation
        )

        assert result.exit_code == 0
        assert result.stdout == "quoted path"

    @pytest.mark.asyncio
    async def test_shell_metacharacters_are_literal_arguments(
        self, sample_invocation: PostAgentInvocation
    ) -> None:
        ...
        assert result.exit_code == 0
        assert json.loads(result.stdout) == [";", "|", ">", "<", "&", "$HOME"]

    @pytest.mark.asyncio
    async def test_empty_command_is_controlled_failure(
        self, sample_invocation: PostAgentInvocation
    ) -> None:
        hook = _make_hook().model_copy(update={"command": ""})

        result = await HookExecutor().run(hook, sample_invocation)

        assert result.exit_code == 1
        assert result.stdout == ""
        assert result.stderr == "hook command is empty"
        assert not result.timed_out

    @pytest.mark.asyncio
    async def test_malformed_quoting_is_controlled_failure(
        self, sample_invocation: PostAgentInvocation
    ) -> None:
        result = await HookExecutor().run(
            _make_hook(command='echo "unterminated'), sample_invocation
        )
    ...
    @pytest.mark.asyncio
    async def test_missing_executable_is_controlled_failure(
        self, sample_invocation: PostAgentInvocation
    ) -> None:
        result = await HookExecutor().run(
            _make_hook(command="/nonexistent/chartreux-hook"), sample_invocation
        )
...
class TestPostAgentHook:
    ...
    async def test_warning_prefers_stderr_on_nonzero_exit(
        ...
        handler = HooksManager([
            _make_hook(
                command=_shell_cmd("echo stdout-msg; echo stderr-msg >&2; exit 1")
            )
        ])
        events = [ev async for ev in _run(handler, HookType.POST_AGENT, ctx)]
    ...
    async def test_warning_falls_back_to_stdout(self, ctx: HookSessionContext) -> None:
        ...
        handler = HooksManager([
            _make_hook(command=_shell_cmd("echo only-stdout; exit 1"))
        ])
        events = [ev async for ev in _run(handler, HookType.POST_AGENT, ctx)]
...
class TestPreToolHook:
    ...
    @pytest.mark.asyncio
    async def test_pre_tool_denial_emits_run_end_before_consumer_stops(
        self, ctx: HookSessionContext
    ) -> None:
        handler = HooksManager([
            _make_tool_hook("guard", _deny_cmd("nope"), type=HookType.PRE_TOOL)
        ])
        events = []
        async for event in _run(
            ...
            events.append(event)
            if isinstance(event, HookToolDenial):
                break

        assert isinstance(events[-1], HookToolDenial)
        assert any(isinstance(event, HookRunEndEvent) for event in events[:-1])
    ...
    async def test_strict_failure_denies(self, ctx: HookSessionContext) -> None:
        handler = HooksManager([
            _make_tool_hook(
                "guard", _shell_cmd("exit 1"), type=HookType.PRE_TOOL, strict=True
            ),
...
class TestPostToolHook:
    ...
    async def test_strict_failure_empties_text(self, ctx: HookSessionContext) -> None:
        handler = HooksManager([
            _make_tool_hook(
                "guard", _shell_cmd("exit 1"), type=HookType.POST_TOOL, strict=True
            ),
...
class TestHookOutputCap:
    ...
    async def test_stdout_capped_at_limit(
        ...
        from chartreux.core.hooks.executor import _MAX_OUTPUT_BYTES
