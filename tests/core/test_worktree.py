...
import chartreux.core.git.repo as git_repo_module
...
def _finish_legacy_prepare(prepared: PreparedWorktree) -> PreparedWorktree:
    if prepared.pending_token is None:
        return prepared
    release_holder(prepared.pending_token)
    return replace(prepared, pending_token=None)


def _prepare(name: str, base: Path, *, branch: str | None = None) -> PreparedWorktree:
    with WorktreeRepository.open(base) as repository:
        return _finish_legacy_prepare(repository.prepare(name, branch=branch))
...
def _prepare_auto(
    ...
    with WorktreeRepository.open(base) as repository:
        return _finish_legacy_prepare(
            repository.prepare_auto(prompt=prompt, suggested_name=suggested_name)
        )
...
_SESSION_TOKENS: dict[tuple[Path, str], OwnershipToken] = {}


def _hold(cwd: Path, session_id: str) -> None:
    if managed := ManagedWorktree.at(cwd):
        acquired = managed.hold(session_id)
        if acquired.token is not None:
            _SESSION_TOKENS[(managed.root, session_id)] = acquired.token
...
def _release(cwd: Path, session_id: str | None = None) -> WorktreeRelease:
    ...
    if session_id is not None and (
        token := _SESSION_TOKENS.pop((managed.root, session_id), None)
    ):
        return managed.remove_if_unheld(
            policy=RemovalPolicy.NORMAL, retiring_token=token
        )
...
def test_sweep_discards_a_reservation_that_never_became_a_worktree(
    ...
    # the redirected CHARTREUX_HOME is one of them, so everything written after it
...
def test_sweep_reclaims_a_named_reservation_whose_add_never_landed(
    ...
    # A live STARTING token protects the interrupted in-process prepare. A real
    # abandoned reservation is reclaimable only after its process (and lock) dies.
    assert _claim(repo, "feature").read() is not None
    assert (_managed_worktree_root(repo) / "feature").exists()
...
def test_auto_worktree_derives_name_and_branch_from_prompt(tmp_path: Path) -> None:
    ...
    assert worktree.branch == "chartreux/fix-the-login-bug"
    assert worktree.created is True
    assert worktree.branch_created is True
    assert Repo(worktree.root).active_branch.name == "chartreux/fix-the-login-bug"
    assert "chartreux/fix-the-login-bug" in (head.name for head in repo.heads)
...
def test_auto_worktree_prefers_a_suggested_name_over_the_prompt(tmp_path: Path) -> None:
    ...
    assert worktree.branch == "chartreux/repair-oauth-redirect"
...
def test_auto_worktree_uses_random_slug_without_prompt(
    ...
    assert worktree.branch == "chartreux/brave-quiet-otter"
...
def test_auto_worktree_suffixes_when_the_name_is_taken(tmp_path: Path) -> None:
    ...
    assert {"chartreux/fix-the-login-bug", "chartreux/fix-the-login-bug-2"} <= branches


def test_auto_worktree_does_not_reuse_an_existing_worktree(tmp_path: Path) -> None:
    _init_repo(tmp_path)
    existing = _prepare(
        "fix-the-login-bug", tmp_path, branch="chartreux/fix-the-login-bug"
    )
...
def test_auto_worktree_skips_a_name_whose_branch_already_exists(tmp_path: Path) -> None:
    repo = _init_repo(tmp_path)
    repo.create_head("chartreux/fix-the-login-bug")
...
def test_auto_worktree_removes_the_branch_git_left_behind(
    ...
    assert "chartreux/fix-the-login-bug" not in {head.name for head in repo.heads}
...
def test_auto_worktree_keeps_a_branch_it_did_not_create(
    ...
    def lose_the_branch_race(*_args: Any, **_kwargs: Any) -> None:
        ...
        repo.create_head("chartreux/fix-the-login-bug", commit=unmerged)
        raise WorktreeError(
            "a branch named 'chartreux/fix-the-login-bug' already exists"
        )
    ...
    head = repo.heads["chartreux/fix-the-login-bug"]
...
def test_inspecting_a_worktree_does_not_execute_a_malicious_fsmonitor_hook(
    ...
    """The same RCE `chartreux.core.system_prompt` already guards against, here.
