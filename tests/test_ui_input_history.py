...
async def test_ui_resumes_arrow_down_after_manual_move(
    ...
    async with chartreux_app.run_test() as pilot:
        inject_history_file(chartreux_app, history_path)
        chat_input = chartreux_app.query_one(ChatInputContainer)
        textarea = chat_input.input_widget
        assert textarea is not None
...
async def test_ui_does_not_intercept_arrow_down_inside_wrapped_single_line_input(
    ...
    async with chartreux_app.run_test(size=(40, 20)) as pilot:
        chat_input = chartreux_app.query_one(ChatInputContainer)
        textarea = chat_input.input_widget
        assert textarea is not None
