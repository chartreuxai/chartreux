...
async def test_act_flushes_batched_messages_with_injection_middleware() -> None:
    ...
    assert observed[0][1] == "You are Chartreux, a super useful programming assistant."
...
async def test_act_handles_user_cancellation_during_streaming() -> None:
    ...
    backend = FakeBackend([
        mock_llm_chunk(content="Preparing "),
        mock_llm_chunk(content="question", tool_calls=[question_tool_call]),
    ])
    agent = build_test_agent_loop(
        config=make_config(enabled_tools=["ask_user_question"]),
        backend=backend,
        enable_streaming=True,
    )
    ...
    async for event in agent.act("Cancel mid stream?"):
        ...
        if isinstance(event, UserInputRequestEvent):
            agent.reject_request(event.request_id, asyncio.CancelledError())

    # The second chunk carries both content ("question") and the tool call;
    ...
    assert "execution interrupted by user" in events[-1].error.lower()
