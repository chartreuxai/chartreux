...
from chartreux.core.agent_loop import CompactionFailedError
from chartreux.core.compaction import parse_previous_user_messages
...
@pytest.mark.asyncio
async def test_auto_compact_emits_compaction_events() -> None:
    backend = FakeBackend([
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(RuntimeError("boom"), RuntimeError, "boom", id="failure"),
        pytest.param(
            asyncio.CancelledError(), asyncio.CancelledError, None, id="cancelled"
        ),
    ],
)
async def test_auto_compact_emits_terminal_events(
...
async def test_compact_uses_compaction_model() -> None:
    ...
    cfg = build_test_vibe_config(
        models=[*make_test_models(auto_compact_threshold=1), compaction],
...
@pytest.mark.asyncio
async def test_compact_uses_fallback_when_primary_returns_tool_call() -> None:
...
@pytest.mark.asyncio
async def test_compact_catalog_selector_fallback_disables_thinking_without_mutation() -> (
    None
):
    config = build_test_vibe_config(compaction_model="glm-5-2")
    config.available_models()["glm-5-2"].thinking = "high"
    backend = _ScriptedBackend([
        [_tool_call_chunk()],
        [mock_llm_chunk(content="<summary>recovered</summary>")],
    ])
    ...
    assert [m.alias for m in backend.requested_models] == ["glm-5-2"] * 2
    assert [m.thinking for m in backend.requested_models] == ["high", "off"]
    assert agent.config.available_models()["glm-5-2"].thinking == "high"


@pytest.mark.asyncio
async def test_compact_glm_5_3_fallback_retains_encodable_thinking() -> None:
    config = build_test_vibe_config(compaction_model="glm-5-2")
    config.available_models()["glm-5-2"].name = "zai-glm-5-3"
    config.available_models()["glm-5-2"].thinking = "low"
    backend = _ScriptedBackend([
        [mock_llm_chunk(content="")],
        [mock_llm_chunk(content="<summary>recovered</summary>")],
    ])
...
@pytest.mark.asyncio
async def test_compact_fallback_failure_raises() -> None:
...
async def test_compact_fallback_rejects_empty_summary_tags() -> None:
    ...
    assert exc_info.value.reason == "tool_call"
    assert all(message.context_boundary != "compaction" for message in agent.messages)
