...
from chartreux.core.agent_loop._loop import AgentLoopStateError, _ActiveTurn
...
from chartreux.core.trusted_folders import WorkspaceTrustStatus, trusted_folders_manager
...
async def test_agent_discovery_follows_the_move(tmp_path: Path) -> None:
    ...
    repo, worktree = _repo_and_worktree(tmp_path)
    (repo / ".chartreux" / "agents").mkdir(parents=True)
    (worktree / ".chartreux" / "agents").mkdir(parents=True)
    agent_loop = _session_in(repo)

    try:
        assert (
            repo / ".chartreux" / "agents"
            in agent_loop.agent_manager._registry.search_paths
        )

        await agent_loop.relocate(worktree)

        paths = agent_loop.agent_manager._registry.search_paths
        assert worktree / ".chartreux" / "agents" in paths
        assert repo / ".chartreux" / "agents" not in paths
    finally:
        trusted_folders_manager.revoke_session_trust(worktree)
        await agent_loop.aclose()
...
async def test_a_subdirectory_move_can_read_the_destination_project_config(
    ...
    # The layer finds `.chartreux` by walking up from the working directory and asks
    ...
    try:
        ...
        assert (
            trusted_folders_manager.trust_status(worktree_root / ".chartreux")
            is WorkspaceTrustStatus.SESSION
        )
    finally:
