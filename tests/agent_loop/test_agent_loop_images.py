...
from chartreux.core.agent_loop import ImagesNotSupportedError
...
def _config_with_both_models() -> ChartreuxConfigSchema:
    models = [
        ModelConfig(
            name="vision-model",
            provider="mistral",
...
def _set_active_model(agent, config: ChartreuxConfigSchema, alias: str) -> None:
    updated = config.model_copy(update={"active_model": alias})
    updated.attach_catalog_snapshot(config.catalog_snapshot)
    updated.attach_committed_model(None)
    set_agent_config(agent, updated)
    agent.committed_model = None


@pytest.mark.asyncio
@pytest.mark.parametrize("enable_streaming", [False, True])
async def test_history_images_reject_nonvision_model_without_stripping(
    ...
    with pytest.raises(ImagesNotSupportedError, match="text-alias"):
        [_ async for _ in agent.act("hi")]

    assert not backend.requests_messages
    # Source-of-truth history is not mutated when the request is rejected.
    assert any(message.images == [png_attachment] for message in agent.messages)
