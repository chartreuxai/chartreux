...
class TestAgentLoopBackgroundTitle:
    ...
    def _patch_generator(monkeypatch: pytest.MonkeyPatch, title: str | None) -> None:
        ...
        monkeypatch.setattr(
            "chartreux.core.session.title_model.generate_session_title", fake_generate
        )
    ...
    async def test_finishes_background_work_when_generation_fails(
        ...
        monkeypatch.setattr(
            "chartreux.core.session.title_model.generate_session_title", fail_generate
        )
        loop = _make_agent_loop(tmp_path)
    ...
    async def test_compaction_refresh_retries_after_failure(
        ...
        monkeypatch.setattr(
            "chartreux.core.agent_loop._loop.is_fast_utility_model", lambda config: True
        )
        monkeypatch.setattr(
            "chartreux.core.session.title_model.generate_session_title", fake_generate
        )
...
class TestAgentLoopTitleTier:
    ...
    async def test_bounds_scheduling_off_the_cheap_model(
        ...
        monkeypatch.setattr(
            "chartreux.core.agent_loop._loop.is_fast_utility_model",
            lambda config: False,
        )
        loop = _make_agent_loop(tmp_path)
        cadence = _RecordingCadence()
