...
from chartreux.core.llm_models import LLMMessage, Role, ToolCall
from chartreux.core.session.session_index import clear_session_index_registry
...
class TestBoundedSessionLoader:
    @pytest.mark.parametrize("final_record", ["{partial", "{partial\n"])
    def test_reads_within_limit_and_recovers_malformed_final_record(
        self, tmp_path: Path, final_record: str
    ) -> None:
        (tmp_path / "meta.json").write_text(json.dumps({"total_messages": 1}))
        (tmp_path / "messages.jsonl").write_text(
            '{"role": "user", "content": "ok"}\n' + final_record
        )

        loaded = SessionLoader.load_session_bounded(tmp_path, byte_limit=1024)

        assert loaded.state.value == "available"
        assert loaded.messages == [{"role": "user", "content": "ok"}]

    def test_exact_boundary_and_one_byte_overflow(self, tmp_path: Path) -> None:
        metadata = json.dumps({"total_messages": 1})
        line = b'{"role":"user","content":"x"}'
        (tmp_path / "meta.json").write_text(metadata)
        (tmp_path / "messages.jsonl").write_bytes(line)

        exact = SessionLoader.load_session_bounded(tmp_path, byte_limit=len(line))
        (tmp_path / "messages.jsonl").write_bytes(line + b" ")
        overflow = SessionLoader.load_session_bounded(tmp_path, byte_limit=len(line))

        assert exact.state.value == "available"
        assert overflow.state.value == "exceeds_limit"

    def test_huge_single_record_returns_limit_outcome(self, tmp_path: Path) -> None:
        (tmp_path / "meta.json").write_text(json.dumps({"total_messages": 1}))
        (tmp_path / "messages.jsonl").write_bytes(b"x" * 4096)

        loaded = SessionLoader.load_session_bounded(tmp_path, byte_limit=128)

        assert loaded.state.value == "exceeds_limit"

    def test_fifo_transcript_file_is_rejected_without_blocking(
        self, tmp_path: Path
    ) -> None:
        (tmp_path / "meta.json").write_text(json.dumps({"total_messages": 1}))
        os.mkfifo(tmp_path / "messages.jsonl")

        started = time.monotonic()
        with pytest.raises(SessionFileContainmentError):
            SessionLoader.load_session_bounded(tmp_path, byte_limit=128)
