...
from chartreux.core.llm.format import APIToolFormatHandler
from chartreux.core.tools.builtins.bash import Bash
from chartreux.core.tools.manager import ToolManager
from chartreux.core.trusted_folders import find_trustable_files, trusted_folders_manager
...
def test_override_from_project_vibe_tools_dir(
    ...
    (``.chartreux/tools/prompts/``), not just explicit ``tool_paths``.
    """
    monkeypatch.setattr(trusted_folders_manager, "is_trusted", lambda _: True)
    prompts_dir = tmp_working_directory / ".chartreux" / "tools" / "prompts"
...
def test_tools_dir_with_prompts_triggers_trust_warning(tmp_path: Path) -> None:
    """A ``.chartreux/tools/`` holding only a description override still surfaces in
    the trust-directory warning.
    """
    prompts_dir = tmp_path / ".chartreux" / "tools" / "prompts"
    prompts_dir.mkdir(parents=True)
    (prompts_dir / "bash.md").write_text("Custom bash description.", encoding="utf-8")

    assert ".chartreux/" in find_trustable_files(tmp_path)
