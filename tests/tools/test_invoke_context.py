...
from chartreux.questions import UserQuestionResult
...
class TestInvokeContext:
    ...
    def test_interaction_requests_can_be_set(self) -> None:
        async def answer(_args: BaseModel) -> BaseModel:
            return UserQuestionResult(answers=[], cancelled=True)

        requests = FakeInteractionRequests(user_input=answer)
...
class TestToolInvokeWithContext:
    ...
    async def test_invoke_with_interaction_requests(
        ...
        ctx = InvokeContext(
            ...
            interaction_requests=FakeInteractionRequests(user_input=answer),
        )
        result = await collect_result(simple_tool.invoke(ctx=ctx, value="test"))
