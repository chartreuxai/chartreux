...
@pytest.mark.asyncio
async def test_streaming_read_stops_at_content_byte_limit(webfetch_small):
    ...
    stream = ChunkStream()
    content, was_truncated = await webfetch_small._read_content(
...
@pytest.mark.asyncio
async def test_total_fetch_deadline_includes_response_setup(webfetch, monkeypatch):
    @asynccontextmanager
    async def slow_fetch(*_args, **_kwargs):
        import asyncio

        await asyncio.sleep(0.05)
        yield httpx.Response(200, text="ok")

    monkeypatch.setattr(webfetch, "_do_fetch", slow_fetch)

    with pytest.raises(ToolError, match="Request timed out after 0.01 seconds"):
        await webfetch._fetch_url("https://example.com", 0.01)
...
@pytest.mark.asyncio
@respx.mock
async def test_same_host_redirect_is_followed(webfetch):
...
@pytest.mark.asyncio
@respx.mock
async def test_cross_host_redirect_is_rejected(webfetch):
    respx.get("https://example.com").mock(
        return_value=httpx.Response(
            302, headers={"Location": "https://internal.example/metadata"}
        )
    )
    redirect_target = respx.get("https://internal.example/metadata").mock(
        return_value=httpx.Response(200, text="sensitive")
    )

    with pytest.raises(ToolError, match="Redirect target is not permitted"):
...
async def test_cloudflare_retry_on_challenge(webfetch):
    ...
    assert second_request.headers["User-Agent"] == "chartreux-cli"
