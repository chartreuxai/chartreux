...
from chartreux.core.tools.base import BaseToolConfig, InvokeContext, ToolError
from chartreux.core.tools.mcp import MCPRegistry, MCPToolResult, RemoteTool
...
class TestMCPRegistryAuthorizationProvider:
    @pytest.mark.asyncio
    async def test_credential_rotation_rediscover_without_public_revision_change(
        self,
    ) -> None:
        ...
        discover = AsyncMock()
        with patch("chartreux.core.tools.mcp.registry.list_tools_http", new=discover):
            for generation in ("one", "two"):
                provider.resolved = _snapshot(
                    f"connection-{generation}",
                    headers={"Authorization": f"Bearer synthetic-{generation}"},
                )
                descriptor = RemoteTool(name="search", description=generation)
                discover.return_value = [descriptor]
                await registry.get_tools_async([_oauth_server()])
                assert registry.descriptors_for("linear") == (descriptor,)
                await registry.get_tools_async([_oauth_server()])
    ...
    async def test_live_proxy_rejection_marks_source_before_emitting_signal(
        ...
        *Assert*: The registry is already NEEDS_AUTH when the event is published.
        ...
        with (
            patch(
                "chartreux.core.tools.mcp.tools.call_tool_http",
                new=AsyncMock(side_effect=_unauthorized()),
            ),
...
class TestMCPHttpAuthorizationProxy:
    ...
    async def test_proxy_resolves_authorization_for_each_call(self) -> None:
        ...
        with patch("chartreux.core.tools.mcp.tools.call_tool_http", new=call):
            results = [
                result
