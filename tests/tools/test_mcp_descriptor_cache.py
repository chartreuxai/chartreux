...
from chartreux.core.config import MCPHttp, MCPStaticAuth, MCPStdio
...
def _server(name: str = "linear") -> MCPHttp:
    return MCPHttp(
        name=name, transport="streamable-http", url=f"https://{name}.example.test/mcp"
    )
...
async def test_legacy_descriptor_cache_evicts_least_recently_used_records(
    ...
    monkeypatch.setattr("chartreux.core.tools.mcp.descriptor_cache._MAX_FILES", 2)
    cache = LegacyMCPDescriptorCache(tmp_path)
    descriptor = (RemoteTool(name="tool"),)
...
async def test_legacy_descriptor_cache_keeps_both_schemas_across_repeated_reads(
    ...
    Direct MCP tools must retain both input and output schemas on cache hits,
    including reads that update the persisted last-used timestamp.
    """
...
@pytest.mark.asyncio
@pytest.mark.parametrize("reuse_registry", [False, True])
@pytest.mark.parametrize(
    "tenants", [("synthetic-a", "synthetic-b"), ("synthetic-b", "synthetic-a")]
)
async def test_private_tenants_never_share_descriptors(
    tmp_path: Path, reuse_registry: bool, tenants: tuple[str, str]
) -> None:
    service = MCPAuthenticationService()
    owners = [MCPRegistry(), MCPRegistry()]
    registry = MCPRegistry()
    discovery = AsyncMock()
    references = []
    with patch("chartreux.core.tools.mcp.registry.list_tools_http", new=discovery):
        for owner, tenant in zip(owners, tenants, strict=True):
            server = _server().model_copy(
                update={"auth": MCPStaticAuth(headers={"X-Tenant": tenant})}
            )
            await service.bind_catalog([server], owner=owner)
            reference = service.reference_for(server, owner=owner)
            references.append(reference)
            if not reuse_registry:
                registry = MCPRegistry()
            registry.configure_authorization(
                RegistryAuthorizationProvider(service),
                {server.name: registry_authorization_ref(reference)},
                descriptor_cache_root=tmp_path,
            )
            descriptor = RemoteTool.model_validate({
                "name": "search",
                "description": tenant,
            ...
            discovery.return_value = [descriptor]
            await registry.get_tools_async([server])
            assert registry.descriptors_for(server.name) == (descriptor,)
            await registry.get_tools_async([server])
        assert discovery.await_count == 2
    assert references[0].server_fingerprint == references[1].server_fingerprint
    assert references[0].descriptor_revision == references[1].descriptor_revision
    assert not list(tmp_path.iterdir())


@pytest.mark.asyncio
@pytest.mark.parametrize("explicit", [False, True])
async def test_stdio_environment_context_changes_rediscover_without_persistence(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, explicit: bool
) -> None:
    service = MCPAuthenticationService()
    registry = MCPRegistry()
    discovery = AsyncMock()
    with patch("chartreux.core.tools.mcp.registry.list_tools_stdio", new=discovery):
        for tenant in ("synthetic-a", "synthetic-b"):
            if not explicit:
                monkeypatch.setenv("HOME", tenant)
            server = MCPStdio(
                name="local",
                transport="stdio",
                command="synthetic",
                env={"TENANT": tenant} if explicit else {},
            )
            await service.bind_catalog([server])
            registry.configure_authorization(
                RegistryAuthorizationProvider(service),
                {
                    server.name: registry_authorization_ref(
                        service.reference_for(server)
                    )
                },
                descriptor_cache_root=tmp_path,
            )
            descriptor = RemoteTool(name="search", description=tenant)
            discovery.return_value = [descriptor]
            await registry.get_tools_async([server])
            assert registry.descriptors_for(server.name) == (descriptor,)
            await registry.get_tools_async([server])
...
@pytest.mark.asyncio
async def test_fallback_environment_token_rotation_rediscover(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    server = _server().model_copy(
        update={"auth": MCPStaticAuth(api_key_env="SYNTHETIC_MCP_TOKEN")}
    )
    registry = MCPRegistry(descriptor_cache_root=tmp_path)
    discovery = AsyncMock()
    with patch("chartreux.core.tools.mcp.registry.list_tools_http", new=discovery):
        for token in ("synthetic-a", "synthetic-b"):
            monkeypatch.setenv("SYNTHETIC_MCP_TOKEN", token)
            descriptor = RemoteTool(name="search", description=token)
            discovery.return_value = [descriptor]
            await registry.get_tools_async([server])
            assert registry.descriptors_for(server.name) == (descriptor,)
            await registry.get_tools_async([server])
