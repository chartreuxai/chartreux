...
from chartreux.core.config import ChartreuxConfigSchema, MCPHttp, MCPStdio
from chartreux.core.tools.base import BaseToolConfig, BaseToolState, InvokeContext
from chartreux.core.tools.mcp import (
    AuthStatus,
    MCPConnectionPool,
    MCPRegistry,
    MCPToolResult,
...
class TestParseCallResult:
    def test_parses_text_content(self):
        ...
        mock_result.content = [SimpleNamespace(text="Hello world")]
    ...
    def test_preserves_structured_and_text(self):
        mock_result = SimpleNamespace()
        mock_result.structuredContent = {"data": "value"}
        mock_result.content = [SimpleNamespace(text="text content")]
...
class TestMCPHttpClient:
    def test_create_vibe_mcp_http_client_uses_vibe_ssl_context(self):
        ...
        with (
            patch(
                "chartreux.core.tools.mcp.tools.build_ssl_context",
                return_value=ssl_context,
            ),
            patch(
                "chartreux.core.tools.mcp.tools.ChartreuxAsyncHTTPClient",
    ...
    async def test_list_tools_http_uses_vibe_mcp_http_client(self):
        ...
        with (
            patch(
                "chartreux.core.tools.mcp.tools.create_vibe_mcp_http_client",
                return_value=fake_client,
            ) as create_client,
            patch("chartreux.core.tools.mcp.tools.streamable_http_client", fake_stream),
            patch(
                "chartreux.core.tools.mcp.tools.ClientSession", _FakeMCPClientSession
            ),
        ):
...
class TestMCPRegistry:
    def _make_http_server(
        ...
        return MCPHttp(name=name, transport="streamable-http", url=url)
    ...
    def test_get_tools_caches_discovery(self):
        ...
        with patch(
            "chartreux.core.tools.mcp.registry.list_tools_http", return_value=[remote]
        ) as discover:
    ...
    def test_count_loaded_excludes_failed_servers(self):
        ...
        with patch(
            "chartreux.core.tools.mcp.registry.list_tools_http",
    ...
    def test_failed_forced_refresh_cannot_fall_back_to_stale_cache(self):
        ...
        with patch("chartreux.core.tools.mcp.registry.list_tools_http", new=discovery):
    ...
    def test_disjoint_server_lists_across_agents(self):
        ...
        with patch(
            "chartreux.core.tools.mcp.registry.list_tools_http", side_effect=discover
        ):
    ...
    async def test_discover_stdio_success(self):
        ...
        with patch(
            "chartreux.core.tools.mcp.registry.list_tools_stdio", return_value=[remote]
        ):
            tools = await registry.get_tools_async([srv])
    ...
    async def test_discover_stdio_failure_returns_none(self):
        ...
        with patch(
            "chartreux.core.tools.mcp.registry.list_tools_stdio",
...
class TestMCPStdioCwd:
    ...
    async def test_list_tools_stdio_passes_cwd_to_params(self):
        with (
            patch("chartreux.core.tools.mcp.tools.stdio_client") as mock_client,
            patch("chartreux.core.tools.mcp.tools.ClientSession") as mock_session_cls,
            patch(
                "chartreux.core.tools.mcp.tools.StdioServerParameters"
            ) as mock_params_cls,
        ):
    ...
    async def test_call_tool_stdio_passes_cwd_to_params(self):
        with (
            ...
            patch("chartreux.core.tools.mcp.tools._parse_call_result") as mock_parse,
        ):
            mock_client.return_value.__aenter__ = AsyncMock(
                return_value=(MagicMock(), MagicMock())
            )
    ...
    async def test_discover_stdio_passes_cwd_to_proxy_class(self):
        ...
        with (
            ...
            patch(
                "chartreux.core.tools.mcp.registry.create_mcp_stdio_proxy_tool_class",
...
class TestMCPDisableFiltering:
    @staticmethod
    def _make_config(
        mcp_servers: list[MCPHttp | MCPStdio] | None = None,
    ...
    def test_disabled_server_excludes_all_tools(self):
        ...
        tm = ToolManager(config_getter=lambda: config, mcp_registry=registry)
...
def _patch_enter(sessions: list[_FakeSession], closed: list[_FakeSession]) -> AsyncMock:
    ...
    async def _enter(stack, params, *, init_timeout):
        session = next(it)
...
class TestMCPConnectionPool:
    ...
    async def test_reuses_connection_across_calls(self):
        ...
        with patch("chartreux.core.tools.mcp.pool.enter_stdio_session", enter):
    ...
    @pytest.mark.asyncio
    async def test_reconnects_only_for_next_explicit_call(self):
        ...
        with patch("chartreux.core.tools.mcp.pool.enter_stdio_session", enter):
            try:
                ...
                result = await pool.call_tool(
                    command=["srv"], tool_name="t", arguments={}
                )
    ...
    @pytest.mark.asyncio
    async def test_first_transport_failure_propagates_without_retry(self):
    ...
    async def test_bind_loop_drops_connections_on_loop_change(self):
        ...
        try:
            ...
            pool._conns["k"] = _StdioConnection(build_stdio_params(["srv"]), None)
            pool._bind_loop()
            assert pool._conns == {}
...
class TestMCPStdioProxyToolPooling:
    ...
    async def test_run_routes_through_pool_when_present(self):
        ...
        with patch("chartreux.core.tools.mcp.tools.call_tool_stdio") as one_shot:
    ...
    async def test_run_falls_back_without_pool(self):
        ...
        with patch(
            "chartreux.core.tools.mcp.tools.call_tool_stdio",
...
class TestMCPConnectionPoolIntegration:
    @pytest.mark.asyncio
    async def test_active_cancellation_terminates_real_subprocess(self, tmp_path):
        script = tmp_path / "active_server.py"
...
for line in sys.stdin:
    ...
    if request.get("method") == "initialize":
        result = {"protocolVersion": "2025-11-25", "capabilities": {"tools": {}},
                  "serverInfo": {"name": "active", "version": "1"}}
    elif request.get("method") == "tools/call":
        marker = Path(sys.argv[1])
        temporary = marker.with_suffix(".tmp")
        temporary.write_text(str(os.getpid()))
        temporary.replace(marker)
        # Never answer and never read stdin again: close must terminate us.
        while True:
            time.sleep(1)
    else:
        continue
...
""")
        ...
        active = asyncio.create_task(
            pool.call_tool(
                command=[sys.executable, str(script), str(started)],
                tool_name="mutate",
                arguments={},
    ...
    @pytest.mark.asyncio
    async def test_real_sdk_lost_response_never_replays_committed_mutation(
        self, tmp_path, record_property
    ):
        script = tmp_path / "lost_response_server.py"
        journal = tmp_path / "mutations.txt"
        script.write_text("""
import os
import sys
from pathlib import Path
...
""")
        pool = MCPConnectionPool()
        command = [sys.executable, str(script), str(journal)]
        try:
            # Preserve observed SDK mapping as runner evidence without assuming
            # EOF necessarily surfaces as an AnyIO exception rather than McpError.
            try:
                async with asyncio.timeout(15):
                    result = await pool.call_tool(
                        command=command,
                        tool_name="mutate",
                        arguments={"operation": "lost"},
                        startup_timeout_sec=5,
                        tool_timeout_sec=5,
                    )
            except TimeoutError:
                pytest.fail("SDK did not settle after server exited")
            except Exception as exc:
                record_property(
                    "lost_response_outcome",
                    f"{type(exc).__module__}.{type(exc).__name__}",
                )
            else:
                record_property("lost_response_outcome", result.model_dump_json())
                assert not result.ok
            assert journal.read_text().splitlines() == ["lost"]
            async with asyncio.timeout(15):
                result = await pool.call_tool(
                    command=command,
                    tool_name="mutate",
            ...
            assert result.ok
            assert journal.read_text().splitlines() == ["lost", "explicit-next"]
        finally:
            await pool.aclose()
