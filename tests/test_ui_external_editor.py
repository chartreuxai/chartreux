...
async def test_ctrl_g_opens_external_editor_and_updates_input(
    ...
    with patch(
        "chartreux.cli.textual_ui.widgets.chat_input.text_area.ExternalEditor"
    ) as MockEditor:
        ...
        async with chartreux_app.run_test() as pilot:
            ...
            with patch.object(chartreux_app, "suspend", mock_suspend):
...
@pytest.mark.asyncio
async def test_ctrl_g_works_with_empty_input(chartreux_app: ChartreuxApp) -> None:
