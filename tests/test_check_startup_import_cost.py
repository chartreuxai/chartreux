...
def test_load_config_parses_commands_with_budgets(tmp_path: Path) -> None:
    content = (
        'strace_code = "import chartreux"\n'
        "slowest_count = 5\n"
        "\n"
        "[[commands]]\n"
        'label = "import chartreux"\n'
        'code = "import chartreux"\n'
        "budget = 68\n"
        "\n"
        "[[commands]]\n"
        'label = "ChartreuxApp"\n'
        'code = "from chartreux.cli.textual_ui.app import ChartreuxApp"\n'
    )

    config = mod._load_config(_write_config(tmp_path, content))

    assert config.slowest_count == 5
    assert config.strace_code == "import chartreux"
    assert len(config.commands) == 2
    assert config.commands[0].label == "import chartreux"
...
def test_find_wheel_returns_single_wheel(tmp_path: Path) -> None:
    ...
    dist_dir.mkdir()
    wheel = dist_dir / "chartreux-1.0.0-py3-none-any.whl"
    wheel.write_text("")
...
def test_find_wheel_raises_on_multiple_wheels(tmp_path: Path) -> None:
    dist_dir = tmp_path / "dist"
    dist_dir.mkdir()
    (dist_dir / "chartreux-1.0.0-py3-none-any.whl").write_text("")
    (dist_dir / "chartreux-1.0.0-py3-none-manylinux.whl").write_text("")
...
def test_measure_strace_skips_on_non_zero_exit(
    ...
    mod._measure_strace(Path("/fake/python"), "import chartreux")
...
def test_measure_importtime_prints_stderr_and_exits_on_failure(
    ...
    with pytest.raises(SystemExit, match="import failed"):
        mod._measure_importtime(Path("/fake/python"), "import chartreux")
