...
from chartreux.cli.autocompletion.file_indexer import FileIndexer
...
def test_rejected_project_git_falls_back_to_filesystem_index(
    ...
    marker = tmp_path / "project-git-ran"
    fake_git = project / "bin" / "git"
    ...
    entries = file_indexer.get_index(project)

    assert {entry.rel for entry in entries} >= {"source.py"}
