...
class TestInitFileLogging:
    ...
    def test_creates_log_directory_if_missing(
        ...
        init_file_logging(log_dir / "chartreux.log", target_logger=test_logger)
    ...
    def test_existing_numbered_backups_repaired_without_touching_lookalikes(
        self, log_file: Path
    ) -> None:
        backup = log_file.with_name(f"{log_file.name}.1")
        lookalike = log_file.with_name(f"{log_file.name}.notes")
        directory = log_file.with_name(f"{log_file.name}.2")
        backup.write_text("old")
        lookalike.write_text("unrelated")
        directory.mkdir()
        backup.chmod(0o666)
        lookalike.chmod(0o666)
        directory.chmod(0o777)

        handler = _ChartreuxFileHandler(log_file, maxBytes=100, backupCount=2)
        handler.close()

        assert backup.stat().st_mode & 0o777 == 0o600
        assert lookalike.stat().st_mode & 0o777 == 0o666
        assert directory.stat().st_mode & 0o777 == 0o777

    def test_symlinked_home_spelling_is_used_for_permission_repair(
        self, tmp_path: Path
    ) -> None:
        real_home = tmp_path / "real-home"
        real_logs = real_home / "logs"
        real_logs.mkdir(parents=True, mode=0o755)
        real_logs.chmod(0o755)
        linked_home = tmp_path / "linked-home"
        linked_home.symlink_to(real_home, target_is_directory=True)
        canonical_log = real_logs / "chartreux.log"
        test_logger = logging.getLogger(f"symlinked-home-{id(tmp_path)}")

        init_file_logging(
            canonical_log,
            repair_path=linked_home / "logs" / "chartreux.log",
            target_logger=test_logger,
        )

        assert canonical_log.exists()
        assert real_logs.stat().st_mode & 0o777 == 0o755
        for handler in list(test_logger.handlers):
            handler.close()
            test_logger.removeHandler(handler)

    def test_symlinked_log_path_is_not_permission_repaired(
        self, tmp_path: Path
    ) -> None:
        ...
        handler = _ChartreuxFileHandler(link, maxBytes=100, backupCount=1)
        handler.close()
...
class TestRuntimeLogLevel:
    ...
    def test_set_log_level_changes_handler_level(
        ...
        handler = next(
            h for h in test_logger.handlers if isinstance(h, _ChartreuxFileHandler)
        )
