...
from chartreux.utils.http import build_ssl_context
...
def test_build_ssl_context_uses_certifi_by_default(monkeypatch):
    ...
    with (
        patch("chartreux.utils.http.certifi.where", return_value="/certifi.pem"),
        patch(
            "chartreux.utils.http.ssl.create_default_context", return_value=mock_ctx
...
def test_build_ssl_context_uses_system_trust_store_when_configured(monkeypatch):
    ...
    with (
        patch(
            "chartreux.utils.http.truststore.SSLContext", return_value=mock_ctx
        ) as truststore_context,
        patch(
            "chartreux.utils.http.ssl.create_default_context"
        ) as create_default_context,
    ):
        build_ssl_context(enable_system_trust_store=True)
...
def test_build_ssl_context_does_not_parse_vibe_config_from_the_environment(monkeypatch):
    monkeypatch.delenv("SSL_CERT_FILE", raising=False)
    monkeypatch.delenv("SSL_CERT_DIR", raising=False)
    monkeypatch.setenv("CHARTREUX_ENABLE_SYSTEM_TRUST_STORE", "true")
    ...
    with (
        ...
        patch("chartreux.utils.http.truststore.SSLContext") as truststore_context,
    ):
...
def test_build_ssl_context_loads_custom_cert_file_with_system_trust_store(
    ...
    mock_ctx = MagicMock(spec=ssl.SSLContext)
    with patch("chartreux.utils.http.truststore.SSLContext", return_value=mock_ctx):
        build_ssl_context(enable_system_trust_store=True)
