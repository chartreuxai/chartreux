...
def _select_option_with_keyboard(
    child: pexpect.spawn, option_text: str, *, max_moves: int = 50
) -> None:
    """Open the focused Select and move until the requested option is rendered."""
    child.send("\r")
    for _ in range(max_moves):
        child.send("\x1b[B")
        try:
            child.expect(ansi_tolerant_pattern(option_text), timeout=0.2)
        except pexpect.TIMEOUT:
            continue
        child.send("\r")
        return
    raise AssertionError(f"Could not reach Select option {option_text!r} by keyboard")
...
def test_empty_home_onboarding_reaches_first_streaming_turn(
    ...
    try:
        ...
        _send_and_wait_for_text(
            child, "\t\r", "Provider ID: generic-openai-style/default"
        )
        ...
        _send_and_wait_for_text(child, "\t" * 16 + "\r", "Provider saved.")
        _send_and_wait_for_text(child, "\t\t\r", "Choose active model")

        child.send("\t")
        _select_option_with_keyboard(child, "onboarding-mock-model")
        _send_and_wait_for_text(child, "\t\r", "Chartreux v", timeout=15)
