...
def _session_permission_factory(
    ...
    arguments = {"command": f'echo "{SESSION_PERMISSION_OUTPUT}"'}
    if request_index == 0:
        return single_tool_call_chunks(
    ...
    return assistant_text_chunks(
        "Second command auto-executed without session permission.", created=210
    )
...
@pytest.mark.parametrize(
    ...
    [pytest.param(_write_file_factory, id="write-file-auto-execution-and-denial")],
    indirect=True,
)
def test_write_file_auto_execution_creates_file_and_denylist_leaves_file_absent(
    ...
    set_tool_denylist(
        Path(os.environ["CHARTREUX_HOME"]),
        "write_file",
    ...
    assert_tool_result_contains(
        ...
        call_id=WRITE_REJECTED_CALL_ID,
        expected="File access denied by a configured path rule",
    )
...
def test_bash_without_allowlist_auto_executes_each_time(
    streaming_mock_server: StreamingMockServer,
    setup_e2e_env: None,
    e2e_workdir: Path,
    spawned_vibe_process: SpawnedChartreuxProcessFixture,
    ...
    with spawned_vibe_process(e2e_workdir) as (child, captured):
        ...
        wait_for_rendered_text(
            child,
            captured,
            needle="Second command auto-executed without session permission.",
            timeout=10,
        )
