...
from tests.e2e.agent_loop_characterization.support import assert_tool_result_contains
...
TOOL_ARGUMENTS = f'{{"command":"echo {PREDICTABLE_OUTPUT}"}}'
...
def test_spawn_cli_auto_executes_bash_and_shows_tool_output(
    ...
    with spawned_vibe_process(e2e_workdir) as (child, captured):
        ...
        wait_for_request_count(
            lambda: len(streaming_mock_server.requests), expected_count=2, timeout=10
        )
