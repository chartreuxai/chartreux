...
from chartreux.acp.user_display_content import USER_DISPLAY_CONTENT_META_KEY
    ...
    running = PublicEffectEntry(
        **_entry_fields("effect-1", PublicEntryGenerationStatus.IN_PROGRESS),
        ...
        detail=FileReadEffectDetail(
            tool_name="custom__read_anything",
...
def test_read_image_uses_call_path_for_summary_only_output() -> None:
    image = PublicEffectEntry(
        **_entry_fields("read-image", PublicEntryGenerationStatus.COMPLETED),
        title="Read image",
        detail=FileReadEffectDetail(
            tool_name="read_image",
            input=FileReadEffectInput(file_path="image.png"),
            display=_display("Reading image.png"),
        ),
        state=CompletedEffectState(
            output=None,
            display=EffectResultDisplay(success=True, message="Read image image.png"),
        ),
    )

    update = replay_history_entry(image)[0]

    assert isinstance(update, ToolCallStart)
    assert update.title == "Reading image.png"
    assert update.locations is not None
    assert update.locations[0].path == str(Path("image.png").resolve())
    assert update.locations[0].field_meta == {"type": "file"}
    assert update.raw_output is None
    assert update.content is not None
    assert _content_texts(update.content) == ["Read image image.png"]
    ...
    entry = PublicEffectEntry(
        **_entry_fields("subagent-effect", PublicEntryGenerationStatus.COMPLETED),
        ...
        detail=SubagentEffectDetail(
            ...
            input=SubagentEffectInput(task="Explore", agent="worker"),
