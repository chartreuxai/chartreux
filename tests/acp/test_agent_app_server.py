...
from chartreux.acp.agent import RETRYING_EXT_METHOD, ChartreuxAcpAgent
from chartreux.acp.exceptions import UnauthenticatedError
from chartreux.app_server.events import SessionUpdated
from chartreux.app_server.local import LocalHarnessHost, LocalHarnessOptions
from chartreux.app_server.models import PublicError, PublicTurnStatus, TurnErrorCode
...
def _agent(backend: FakeBackend) -> tuple[ChartreuxAcpAgent, FakeClient]:
...
async def test_new_session_without_a_key_is_unauthenticated(
    ...
    *Assert*: ``UnauthenticatedError`` naming the provider.
    ...
    async def start_session(options: LocalHarnessOptions) -> AppServerSession:
        return await harness_host.start(replace(options))
...
async def test_retry_notification_reaches_the_wire_underscore_prefixed() -> None:
    ...
    connection = AgentSideConnection(
        lambda _client: ChartreuxAcpAgent(), agent_writer, agent_reader, listening=False
    )
    try:
...
def test_acp_runtime_adapter_has_no_direct_core_dependency() -> None:
    root = Path(__file__).parents[2] / "chartreux" / "acp"
    runtime_files = [
        root / "agent.py",
        root / "session.py",
    ...
    assert "chartreux.acp.tools" not in contents
