...
) -> tuple[ChartreuxAcpAgent, FakeClient]:
...
async def test_ask_user_question_round_trips_through_acp_elicitation(
    ...
    try:
        ...
        assert captured_options["callback_kinds"] == ["user_input"]
...
@pytest.mark.asyncio
async def test_cancel_during_elicitation_resolves_prompt_and_allows_next_prompt(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    agent, client = _agent(
        FakeBackend([
    ...
    elicitation_started = asyncio.Event()
    elicitation_cancelled = asyncio.Event()

    async def hanging_elicitation(*args: object, **kwargs: object) -> None:
        del args, kwargs
    ...
    monkeypatch.setattr(client, "create_elicitation", hanging_elicitation)
...
async def test_tools_are_gated_by_elicitation_capability() -> None:
    ...
    client advertises no callbacks; with it, user-input callbacks are available.
    """
    ...
    try:
        ...
        assert captured_disabled["disabled_tools"] == ["ask_user_question"]
        assert captured_disabled["callback_kinds"] == []
    finally:
    ...
    try:
        ...
        assert captured_enabled["callback_kinds"] == ["user_input"]
    finally:
