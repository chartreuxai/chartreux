...
BROWSER_AUTH_DESCRIPTION = "Sign into Chartreux through your Mistral AI Studio account."
...
def vibe_home_dir(tmp_path: Path) -> Path:
    return tmp_path / ".chartreux"


async def _spawn_chartreux_acp(env: dict[str, str]) -> asyncio.subprocess.Process:
...
def _build_env(vibe_home_dir: Path, *, include_api_key: bool) -> dict[str, str]:
    ...
    env["CHARTREUX_HOME"] = str(vibe_home_dir)
...
async def _connect_and_initialize(
    *,
    ...
    proc = await _spawn_chartreux_acp(env)
...
async def test_vibe_acp_initialize_and_new_session(vibe_home_dir: Path) -> None:
    ...
    try:
        ...
        assert initialize_response.agent_info.name == "chartreux"
        assert initialize_response.agent_info.title == "Chartreux"
        ...
        assert config_schema["schema"]["title"] == "ChartreuxConfigSchema"
...
async def test_vibe_acp_bootstraps_default_files(vibe_home_dir: Path) -> None:
    ...
    assert (vibe_home_dir / "chartreuxhistory").is_file()
...
def test_acp_agent_import_does_not_load_gitpython() -> None:
    ...
    # found. The ACP startup path (e.g. `chartreux-acp --version`) must not import it,
    # so binaries run on machines without git. See teleport lazy import.
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            "import sys, chartreux.acp.agent; "
