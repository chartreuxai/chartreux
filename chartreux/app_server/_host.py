...
from chartreux.app_server._state import build_stored_public_state, history_page
...
from chartreux.app_server.models import ConfigIssue, IdleSessionStatus, PublicSession
...
from chartreux.core.git.errors import (
    GitError,
...
from chartreux.core.session.resume_sessions import (
    ResumeSessionInfo,
    list_local_resume_sessions,
)
from chartreux.core.session.saved_sessions import (
...
class HostRequestHandler:
    ...
    def _read_session(
        self, params: SessionReadParams, config: ChartreuxConfigSchema
    ) -> SessionReadResponse:
    ...
    def _history_list(
        self, params: SessionHistoryListParams, config: ChartreuxConfigSchema
    ) -> SessionHistoryListResponse:
        messages, metadata = self._load_session(params.session_id, config)
    ...
    def _get_session_history(
        self, params: SessionHistoryGetParams, config: ChartreuxConfigSchema
    ) -> SessionHistoryGetResponse:
        messages, metadata = self._load_session(params.session_id, config)
    ...
    async def _relocate_session(
        self, params: SessionRelocateParams, config: ChartreuxConfigSchema
    ) -> SessionRelocateResponse:
    ...
    def _load_session(
        self, session_id: str, config: ChartreuxConfigSchema
    ...
    async def _load_config(self, cwd: str | None) -> ChartreuxConfigSchema:
...
def config_schema_response() -> ConfigSchemaReadResponse:
    schema = ChartreuxConfigSchema.model_json_schema(
...
def project_session_list(
    config: ChartreuxConfigSchema, params: SessionListParams
) -> SessionListResponse:
...
def _continue_session_id(
    config: ChartreuxConfigSchema, filtered: list[ResumeSessionInfo]
) -> str | None:
...
def _session_cursor_index(sessions: list[ResumeSessionInfo], cursor: str | None) -> int:
    ...
    try:
        updated_at, session_id = base64.urlsafe_b64decode(padded).decode().split("\0")
...
def worktree_remove_response(cwd: Path) -> WorkspaceWorktreeRemoveResponse:
    ...
    try:
        release = managed.remove_if_unheld()
...
def _unreadable_checkout(
    ...
    # No path in the message: keeps repo-local paths out of shared logs. The
    # response carries the reason, which reaches only this session's own renderer.
