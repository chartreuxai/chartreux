...
_DISABLE_KEYRING_ENV_VAR = "CHARTREUX_TEST_DISABLE_KEYRING"
...
def _get_uncached_password(username: str) -> str | None:
    try:
        return _get_password(_KEYRING_SERVICE, username)
    except _PasswordNotFoundError:
        return None


def get_api_key_from_keyring_uncached(env_key: str) -> str | None:
    """Read a credential without caching, propagating storage failures."""
    if not env_key:
        return None
    if _is_keyring_disabled():
        return None
    return _get_uncached_password(env_key)


def get_api_key_from_keyring(env_key: str) -> str | None:
    """Read a Chartreux credential, treating unavailable keyring backends as empty."""
    try:
        return get_api_key_from_keyring_uncached(env_key)
    except KeyringError:
        return None
