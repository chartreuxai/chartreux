...
def _build_ssl_context(
    *,
    ...
    ssl_cert_dir: str | None,
) -> ssl.SSLContext:
    if enable_system_trust_store:
...
class _SSLContextBuilder(Protocol):
    def __call__(
        self, *, enable_system_trust_store: bool = False
    ) -> ssl.SSLContext: ...
...
def _build_ssl_context_from_environment(
    *, enable_system_trust_store: bool = False
) -> ssl.SSLContext:
    """Build a cached TLS context for the current certificate environment."""
...
build_ssl_context = cast(_SSLContextBuilder, _build_ssl_context_from_environment)
build_ssl_context.cache_clear = _build_ssl_context.cache_clear
build_ssl_context.cache_info = _build_ssl_context.cache_info
...
class ChartreuxAsyncHTTPClient(httpx.AsyncClient):
    """HTTPX client that works around HTTPX's CIDR NO_PROXY limitation."""

    def __init__(
        self, *, enable_system_trust_store: bool = False, **kwargs: Any
    ) -> None:
        kwargs["trust_env"] = False
        kwargs.setdefault(
            "verify",
            build_ssl_context(enable_system_trust_store=enable_system_trust_store),
        )
        if (
...
def get_user_agent(backend: str | None) -> str:
    user_agent = f"Chartreux/{__version__}"
    if backend == "mistral":
