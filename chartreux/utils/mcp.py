...
type MCPAddTransport = Literal["streamable-http"]


def parse_mcp_add_transport(value: str) -> MCPAddTransport:
    if value == "streamable-http":
        return value
    raise ValueError("MCP server transport must be: streamable-http.")
...
def format_tool_display_description(
    ...
    Remote MCP descriptions are free-form and routinely span
