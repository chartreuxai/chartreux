...
from chartreux.utils.paths import get_chartreux_home, get_chartreux_home_literal
...
class _ChartreuxFileHandler(RotatingFileHandler):
    def __init__(
        self, filename: Path, *args: Any, repair_path: Path | None = None, **kwargs: Any
    ) -> None:
        self._repair_path = (repair_path or filename).expanduser()
        super().__init__(filename, *args, **kwargs)
        self._repair_backups()

    def _repair_backups(self) -> None:
        for index in range(1, self.backupCount + 1):
            backup = self._repair_path.with_name(f"{self._repair_path.name}.{index}")
            restrict_private_file(backup)

    def _open(self) -> TextIOWrapper:
        stream = super()._open()
        restrict_private_path(self._repair_path)
        return stream
...
def init_file_logging(
    ...
    *,
    ...
    literal_log_file = (repair_path or log_file).expanduser()
    handler_filename = Path(os.path.abspath(log_file.expanduser()))
    targets = [target_logger]

    shared_handler: _ChartreuxFileHandler | None = None
    for candidate in targets:
        if any(
            isinstance(handler, _ChartreuxFileHandler)
            and Path(handler.baseFilename) == handler_filename
            for handler in candidate.handlers
        ):
            continue
        if shared_handler is None:
            try:
                if handler_filename.is_relative_to(get_chartreux_home()):
                    ensure_private_directory(get_chartreux_home_literal())
            except (OSError, RuntimeError):
                pass
            ensure_private_directory(literal_log_file.parent)
            max_bytes = int(os.environ.get("LOG_MAX_BYTES", 10 * 1024 * 1024))
            shared_handler = _ChartreuxFileHandler(
                handler_filename,
                maxBytes=max_bytes,
                backupCount=0,
                encoding="utf-8",
                repair_path=literal_log_file,
            )
...
def _vibe_file_handlers(
    target_logger: logging.Logger = logger,
) -> list[_ChartreuxFileHandler]:
    return [h for h in target_logger.handlers if isinstance(h, _ChartreuxFileHandler)]
