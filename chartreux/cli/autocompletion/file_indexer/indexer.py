...
from chartreux.cli.autocompletion.file_indexer.watcher import WatchController
...
class FileIndexer:
    ...
    def refresh(self) -> None:
        ...
        with self._lock:
            self._store.clear()
            self._store.reset_ignore_rules()
    ...
    def _start_background_rebuild(self, root: Path) -> None:
        with self._rebuild_lock:  # one rebuild per root
            ...
            cancel_event = Event()
            done_event = Event()
            task = _RebuildTask(cancel_event=cancel_event, done_event=done_event)
            self._active_rebuilds[root] = task

        try:
            self._rebuild_executor.submit(self._rebuild_worker, root, task)
        except RuntimeError:
            with self._rebuild_lock:
    ...
    def _rebuild_worker(self, root: Path, task: _RebuildTask) -> None:
        try:
            if task.cancel_event.is_set():  # cancelled before work began
                with self._rebuild_lock:
                    if self._active_rebuilds.get(root) is task:
                        self._active_rebuilds.pop(root, None)
                return
            ...
            while not task.cancel_event.is_set():
                with self._lock:
                    dirty_generation = self._store.dirty_generation
                rebuilt = self._store.build(
                    root, should_cancel=lambda: task.cancel_event.is_set()
                )
                if rebuilt is None or task.cancel_event.is_set():
                    return

                with self._lock:  # publish only completed, current rebuilds
                    with self._rebuild_lock:
                        if task.cancel_event.is_set() or self._target_root != root:
                            return
                    if self._store.publish(
                        rebuilt, expected_generation=dirty_generation
                    ):
                        break
    ...
    def _handle_watch_changes(
        ...
        with self._lock:  # make watcher ignore stale roots
            if self._store.root != root:
                return
            should_rebuild = self._store.apply_changes(normalized)
        if should_rebuild:
            self._start_background_rebuild(root)
