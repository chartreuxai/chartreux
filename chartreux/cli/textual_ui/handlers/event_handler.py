...
class EventHandler:
    def __init__(
        ...
        self._tool_group_call_positions: dict[str, tuple[ToolGroup, int]] = {}
        self._current_tool_group_next_position = 0
        self._pending_error_results: list[ToolResultMessage] = []
        self._pending_error_positions: dict[
            ToolResultMessage, tuple[ToolGroup, int]
        ] = {}
        ...
        if existing is not None:
            ...
            if not entry_keeps_tool_group(entry):
                ...
                self._tool_group_call_positions[entry.id] = (
                    group,
                    self._current_tool_group_next_position,
                )
                self._current_tool_group_next_position += 1
    ...
    async def _handle_effect_completed(
        ...
        if position is not None:
            group, timeline_index = position
            group.record_effect(timeline_index, entry.state)
            if effect_state_is_failure(entry.state):
                if group.failure_is_muted(timeline_index):
                    result.set_error_escalation(escalate=False)
                else:
                    self._pending_error_results.append(result)
                    self._pending_error_positions[result] = (group, timeline_index)
            elif (
                isinstance(entry.state, CompletedEffectState)
                and entry.state.display.success
            ):
                # The group header remains the last timeline call's status;
                # this only resolves earlier individual failures in the group.
                self._resolve_pending_errors_for_group(group, timeline_index)
            if not any(
                pending_group is group
    ...
    def _resolve_pending_errors_for_group(
        self, group: ToolGroup, successful_position: int
    ) -> None:
        """Mute earlier failures resolved by a later successful group call."""
        retained: list[ToolResultMessage] = []
        for result in self._pending_error_results:
            position = self._pending_error_positions.get(result)
            if (
                position is not None
                and position[0] is group
                and position[1] < successful_position
            ):
                self._pending_error_positions.pop(result, None)
            else:
                retained.append(result)
        self._pending_error_results = retained
    ...
    def _resolve_pending_errors(self, *, escalate: bool) -> None:
        ...
        self._pending_error_positions.clear()
