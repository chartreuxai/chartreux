...
def build_history_widgets(
    ...
    *,
    ...
    are anchored to the oldest entry ID.  The latter remains valid when older
    pages are prepended and indices are shifted.  Each batch is built
    independently, so a run that crosses a pagination boundary remains two
    consistently-rendered groups until pagination can merge mounted widgets.
    """
    widgets: list[Widget] = []
