...
class MCPApp(Container):
    ...
    def _set_highlighted_disabled(self, *, disabled: bool) -> None:
        ...
        discovery would flash as enabled and revert to its real status on the
        next repaint. Leave the status to the authoritative refresh that
        follows the toggle.
        """
        if self._viewing_name is not None:
        ...
        self.post_message(self.MCPToggled(name=target.name, disabled=disabled))
        self._rebuild_preserving_scroll()
    ...
    def _set_highlighted_tool_disabled(self, *, disabled: bool) -> None:
        ...
        self.post_message(
            self.MCPToggled(name=source.name, disabled=disabled, tool_name=tool.name)
        )
        self._rebuild_preserving_scroll()
    ...
    def _add_source_group(
        ...
        for source in sources:
            ...
            if source.status is MCPSourceStatus.UNAVAILABLE and total == 0:
    ...
    def _find_source(self, name: str | None) -> MCPSourceSummary | None:
        if name is None:
            return None
        return next(
            (source for source in self._state.sources if source.name == name), None
        )

    def _viewing_source(self) -> MCPSourceSummary | None:
        return self._find_source(self._viewing_name)
...
def _source_from_option_id(value: str) -> str | None:
    prefix = "server:"
