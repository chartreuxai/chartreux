...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="chartreux mcp", description="Manage MCP server configuration."
    )
    ...
    add_parser.add_argument(
        "--url", help="Remote MCP server URL (streamable-http transport)."
    )
    add_parser.add_argument(
...
def _build_remote_add_command(args: argparse.Namespace) -> _MCPAddCommand:
    ...
    if not args.url:
        raise MCPCommandError("--url is required for streamable-http transports.")
...
async def _add_mcp_server(command: _MCPAddCommand) -> str:
    ...
    try:
        ...
        print(
            f"chartreux mcp add: OAuth login failed: {exc}\n"
    ...
    if not isinstance(persisted, MCPHttp) or not isinstance(persisted.auth, MCPOAuth):
        return message
