...
def render_braille_styled(
    dot_coords: Iterable[complex],
    width: int,
    height: int,
    highlight_cells: set[tuple[int, int]] | None = None,
    highlight_style: Style | None = None,
) -> Text:
    """Like render_braille, but returns a Text with styled character cells.

    highlight_cells is a set of (char_col, char_row) positions to style.
    """
    dots_matrix: list[list[list[int]]] = [
        [[] for _ in range(math.ceil(width / 2))] for _ in range(math.ceil(height / 4))
    ]
    ...
    text = Text()
    for char_row, row in enumerate(dots_matrix):
        for char_col, char_dots in enumerate(row):
            style = (
                highlight_style
                if highlight_style is not None
                and highlight_cells is not None
                and (char_col, char_row) in highlight_cells
                else None
            )
            text.append(_braille_char_from_dot_indices(char_dots), style=style)
        if char_row < len(dots_matrix) - 1:
            text.append("\n")
