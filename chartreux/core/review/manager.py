...
from chartreux.core.checkpoints import (
    AgentTurn,
    Checkpointer,
    Decision,
    FileState,
...
from chartreux.utils.io import decode_safe
...
class ReviewManager:
    ...
    def _decide(self, target: ReviewTarget, decision: Decision) -> list[str]:
        ...
        # Commit the in-memory decision only if all requested writes succeed.
        # Files are persisted independently, so failures report any earlier writes
        # honestly; the context manager cannot roll those filesystem changes back.
        try:
            with self._checkpointer.atomic():
                ...
                for path in unique:
                    try:
                        if self._persist(path):
                            written.append(path)
                    except ReviewError as exc:
                        failed.append((path, str(exc)))
                if failed:
                    succeeded = ", ".join(written) if written else "none"
                    failures = "; ".join(f"{path}: {error}" for path, error in failed)
                    raise ReviewError(
                        f"Review persistence partially failed; wrote: {succeeded}; "
                        f"failed: {failures}"
                    )
                return unique
        except (FileStateError, TurnStateError) as exc:
            raise ReviewError(str(exc)) from exc

    def _preflight_target(self, target: ReviewTarget) -> None:
        """Validate explicit target references before any acting boundary."""
        path: str | None = None
        owner: Owner | None = None
        regions: tuple[RegionRef, ...] = ()
        match target:
            case RegionTarget(target_path, version_index, ordinal):
                path = target_path
                regions = ((version_index, ordinal),)
            case RegionsTarget(target_path, target_regions):
                path = target_path
                regions = target_regions
            case FileTarget(target_path):
                path = target_path
            case ScopeFileTarget(target_owner, target_path):
                owner = target_owner
                path = target_path
            case ScopeTarget(target_owner):
                owner = target_owner
            case AllTarget() | LastTurnsTarget():
                return

        history = self._checkpointer.view()
        if owner is not None:
            scope_exists = owner in history.scopes
            if isinstance(owner, AgentTurn):
                scope_exists = scope_exists or history.has_turn(owner.turn_id)
            if not scope_exists:
                raise ReviewError(
                    f"Review target rejected: unknown scope in this session: {owner}"
                )

        if path is None:
            return
        if not self._checkpointer.has_checkpoint(path):
            raise ReviewError(
                f"Review target rejected: {path!r} has no checkpoint in this session"
            )
        if owner is not None and not any(
            region_owner == owner
            for _region_id, region_owner in history.owned_hunks(path)
        ):
            raise ReviewError(
                f"Review target rejected: {path!r} is not tracked in scope {owner}"
            )

        valid = {rid for rid, _deps in history.hunk_order(path)}
        unknown = [
            RegionId(version_index, ordinal) for version_index, ordinal in regions
        ]
        unknown = [region_id for region_id in unknown if region_id not in valid]
        if unknown:
            refs = ", ".join(str(region_id) for region_id in unknown)
            raise ReviewError(
                f"Review target rejected: unknown region(s) for {path}: {refs}"
            )
