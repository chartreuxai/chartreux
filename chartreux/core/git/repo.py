...
# A repository can name executable hooks and an fsmonitor command. Both must be
# neutralized for every pre-trust command. The hooks path is absolute and absent
# so it cannot resolve relative to the untrusted worktree.
_NO_HOOKS_CONFIG = "core.hooksPath=/nonexistent-chartreux-disabled-git-hooks"
...
def sanitized_git(repo: Repo) -> Git:
    """Return the shared GitPython command handle sanitized for one command.

    GitPython clears command overrides after invocation, so callers must call
    this helper separately for every command rather than retaining its result.
    """
    command = repo.git
