...
from chartreux.core.llm.failures import RequestRetryBudget, classify_failure
...
def _is_retryable_http_error(error: Exception) -> bool:
    return classify_failure(error).retry_eligible
...
def _parse_retry_after(error: Exception) -> float | None:
    ...
    backoff.
    """
    return classify_failure(error).retry_after
...
def async_retry[T, **P](
    ...
    """
    if tries is None and max_elapsed_time is None and budget is None:
        raise ValueError(
        ...
        async def wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
            ...
            for attempt in attempts:
                try:
                    ...
                    budget_spent = (
                        active_budget is not None and active_budget.exhausted
                    ) or (
                    ...
                    if active_budget is not None and not active_budget.allows_delay(
                        current_delay
                    ):
                        raise e
                    logger.warning(
                    ...
                    if (active_budget is not None and active_budget.exhausted) or (
                        max_elapsed_time is not None
                        and time.monotonic() - start >= max_elapsed_time
                    ):
                        raise e
