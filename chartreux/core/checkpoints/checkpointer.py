...
from chartreux.core.checkpoints._events import (
    _Decide,
    _Edit,
    _Event,
    _rid_key,
...
class Checkpointer:
    ...
    **turns**, **manual edits** (each its own owner slot) and **decisions**
    ...
    def has_checkpoint(self, path: str) -> bool:
        """Whether ``path`` was captured by a checkpoint in this session."""
        return path in History(self._events).tracked_paths
    ...
    def atomic(self) -> Iterator[None]:
        """Roll the in-memory log back to its prior state if the body raises.

        This does not roll back filesystem or other external side effects made by
        the body.
        """
