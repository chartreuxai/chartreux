...
def list_local_resume_sessions(
    config: ChartreuxConfigSchema, cwd: str | None
...
def session_latest_messages(
    sessions: list[ResumeSessionInfo], config: ChartreuxConfigSchema
) -> dict[str, str]:
...
def resume_directories(config: ChartreuxConfigSchema) -> tuple[Path, ...]:
    ...
    """
    try:
        sessions = SessionLoader.list_sessions(
            config.session_logging, cwd=None, strict_io=True
        )
    except OSError:
