...
def _entry_from_payload(payload: Any) -> _Entry | None:
    ...
    if not (
        ...
        and isinstance(messages_mtime_ns, int)
        and isinstance(messages_size, int)
    ...
    return _Entry(
        mtime_ns=mtime_ns,
        messages_mtime_ns=messages_mtime_ns,
...
class SessionIndex:
    ...
    The `meta.json` and `messages.jsonl` files remain the source of truth; this
    index is a cache that reconciles against them on every read using cheap
    `stat` calls and only re-reads directories whose source files changed. Any
    structural corruption of the persisted index triggers a full rebuild from
    ...
    def list(
        self, cwd: str | None = None, *, strict_io: bool = False
    ) -> list[SessionInfo]:
        """Return cached session infos, reconciled against disk, filtered by cwd."""
        with self._lock:
            self._reconcile(strict_io=strict_io)
    ...
    def _reconcile(self, *, strict_io: bool = False) -> None:
        ...
        for session_dir in self._save_dir.glob(self._pattern):
            ...
            try:
                ...
                messages_stat = messages_path.stat()
            except OSError:
                if strict_io:
                    raise
                continue
            ...
            if (
                ...
                and cached.mtime_ns == meta_stat.st_mtime_ns
                and cached.messages_mtime_ns == messages_stat.st_mtime_ns
                and cached.messages_size == messages_stat.st_size
            ):
                continue
            ...
            entry = self._read_entry(
                ...
                messages_stat.st_mtime_ns,
                messages_stat.st_size,
                strict_io=strict_io,
            )
            if entry is None:
    ...
    def _persist(self) -> None:
        ...
        payload = {
            name: {
                ...
                "messages_mtime_ns": entry.messages_mtime_ns,
                "messages_size": entry.messages_size,
                **entry.info,
            }
...
_INDEXES: dict[tuple[str, str], SessionIndex] = {}


def session_index_for(config: SessionLoggingConfig) -> SessionIndex:
    """Return the process-wide index for this save_dir and prefix."""
    key = (config.save_dir, config.session_prefix)
    with _REGISTRY_LOCK:
