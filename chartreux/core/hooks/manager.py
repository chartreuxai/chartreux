...
from chartreux.core.hooks._handler import (
    HookHandler,
    HookOutputError,
    HookRetryState,
    _failure_reason,
...
from chartreux.core.hooks._post_agent import PostAgentHandler
from chartreux.core.hooks._post_tool import PostToolHandler
from chartreux.core.hooks._pre_tool import PreToolHandler
...
class HooksManager:
    ...
    async def run(self, invocation: HookInvocation) -> AsyncGenerator[_HookYield]:
        ...
        for hook in hooks:
            ...
            result = await self._executor.run(hook, current)

            action = self._process_hook_result(handler, hook, current, result)
            deferred_events: list[_HookYield] = []
            for ev in action.events:
                if action.should_break and not isinstance(ev, HookEndEvent):
                    deferred_events.append(ev)
                elif isinstance(ev, HookEndEvent):
                    yield ev.model_copy(
            ...
            if action.should_break:
                # Pre-tool consumers stop as soon as they receive a terminal
                # denial, so complete the UI lifecycle before yielding it.
