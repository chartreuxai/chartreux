...
Use this when the user asks you to create, update, or delete a Chartreux skill.
...
the user which they want when it is not obvious. Creating or changing a skill also
requires filesystem authorization for its directory; select the scope before
...
- `name` (required): lowercase letters, numbers, and hyphens only
  ...
  name). Do not reuse a built-in skill name (e.g. `chartreux`) or a name already
...
`defaultEnabled`) are not part of Chartreux's schema and are ignored.
...
- Create: make `<skills-dir>/<slug>/SKILL.md` with valid frontmatter, where
  `<skills-dir>` is the directory for the chosen scope (`.chartreux/skills/` for
  project, `~/.chartreux/skills/` for global). Add support files under the same
...
Writing under a skills directory requires filesystem authorization for the target
path. Use the normal `write_file` / `edit` tools only when that authorization
allows the operation; do not promise a permission prompt or a successful write.
After changes, tell the user they can run `/reload` to pick up the new or edited
skill without restarting. Keep the final summary brief; do not paste full file
contents unless the user asks."""

SKILL = SkillInfo(
    name="skill-creator",
    description=(
        "Load this skill before creating, updating, or deleting a Chartreux skill, "
        "or whenever you plan to add or modify SKILL.md files under a skills directory. "
        "It explains Chartreux's SKILL.md frontmatter, discovery order, support files, "
