"""Shared LLM failure contracts.

Dispatch ownership for the credential-diagnostics rollout:
D6 owns this module, ``utils/api_keys.py``, app-server error classification,
and their focused tests. D7 owns ``backend/generic.py`` and generic/reasoning/
Anthropic adapter tests. D8 owns ``backend/mistral.py`` and Mistral backend tests.
D9 owns ``backend/openai_responses.py`` and its adapter tests. D10 owns
``cli/textual_ui/app.py`` and textual-UI retry tests. D11 owns the legacy
app-server runtime/session backend and app-server authentication/turn tests.
"""
...
from chartreux.core.llm.failures import FailureInfo, classify_failure
from chartreux.core.llm_models import AvailableTool, LLMMessage, StrToolChoice
...
class ModelCall:
    ...
    def payload_summary(self) -> PayloadSummary:
        return PayloadSummary(
            ...
            approx_chars=sum(len(message.content or "") for message in self.messages),
...
class BackendError(RuntimeError):
    def __init__(
        ...
        *,
        ...
        self.failure_info: FailureInfo = classify_failure(self)
    ...
    def _fmt(self) -> str:
        if self.status in {HTTPStatus.UNAUTHORIZED, HTTPStatus.FORBIDDEN}:
            origin = (
        ...
        if self.is_invalid_model:
            lines = [
                ...
                "or update the model name in config.toml.",
            ]
