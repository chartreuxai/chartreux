...
from chartreux.core.utils import StreamHTTPError
...
# Compatible providers can add fields or vary fields that Chartreux does not read. Validate
...
class _OpenAIResponsesStreamParser:
    def __init__(self, model_call: ModelCall | None = None) -> None:
...
class OpenAIResponsesAdapter(APIAdapter):
    ...
    THINKING_LEVELS: ClassVar[dict[str, str | None]] = OPENAI_RESPONSES_THINKING_LEVELS

    def __init__(self, model_call: ModelCall | None = None) -> None:
        self._stream_parser = _OpenAIResponsesStreamParser(model_call)
    ...
    @classmethod
    def _map_reasoning_effort(cls, thinking: str, model_name: str) -> str:
        levels = get_thinking_levels("generic", "openai-responses", model_name)
        assert levels is not None
        wire_value = get_thinking_wire_value(levels, thinking)
    ...
    def _convert_messages(self, messages: Sequence[LLMMessage]) -> list[dict[str, Any]]:
        ...
        for msg in messages:
            match msg.role:
                ...
                case Role.tool:
                    output: str | list[dict[str, str]] = msg.content or ""
                    if msg.images:
                        output = [
    ...
    def prepare_request(
        ...
        *,
        ...
        if not getattr(provider, "supports_tool_result_images", True):
            messages = project_tool_images(messages)
    ...
    def parse_response(
        ...
        if not event_type and ("output" in data or data.get("object") == "response"):
            response_data = _RESPONSES_OBJECT_ADAPTER.validate_python(data)
            if response_data.get("status") == "failed" or response_data.get("error"):
                return self._stream_parser._on_error({
                    "type": "error",
