...
from chartreux.core.llm.backend._image import to_base64 as _to_base64
...
class AnthropicAdapter(APIAdapter):
    ...
    THINKING_LEVELS: ClassVar[dict[str, str | None]] = ANTHROPIC_THINKING_LEVELS
    ...
    def _apply_thinking_config(
        ...
        *,
        ...
        levels = get_thinking_levels("generic", "anthropic", model_name)
        assert levels is not None
        wire_level = get_thinking_wire_value(levels, effective_level)
        assert wire_level is not None
        payload["thinking"] = {"type": "adaptive", "display": "summarized"}
        payload["output_config"] = {"effort": wire_level}
        payload["max_tokens"] = (
    ...
    def _parse_streaming_event(self, data: dict[str, Any]) -> LLMChunk:
        ...
        if event_type == "error":
            ...
            if error_status is not None and self._model_call is not None:
                raise BackendError(
