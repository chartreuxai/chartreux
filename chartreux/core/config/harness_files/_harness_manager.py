...
from chartreux.core.config.harness_files._paths import (
    GLOBAL_AGENTS_DIR,
...
class HarnessFilesManager:
    ...
    _session_trust_grants: tuple[Path, ...] = ()
    ...
    def trust_for_session(self, path: Path) -> HarnessFilesManager:
        """Grant temporary trust owned by this session manager."""
        resolved = path.resolve()
        self.trust_store.trust_for_session(resolved)
        return replace(
            self, _session_trust_grants=(*self._session_trust_grants, resolved)
        )

    def revoke_session_trusts(self) -> None:
        """Revoke only the temporary grants this session manager owns."""
        self.trust_store.revoke_session_trusts(self._session_trust_grants)
    ...
    def cwd_is_user_config_home(self) -> bool:
        """True when the cwd's ``.chartreux`` dir is the user config dir itself."""
        return (
            self._effective_cwd / ".chartreux"
        ).resolve() == CHARTREUX_HOME.path.resolve()
    ...
    def config_file(self) -> Path | None:
        ...
        if workdir is not None:
            candidate = workdir / ".chartreux" / "config.toml"
            if candidate.is_file():
                return candidate
        if "user" in self.sources:
            return CHARTREUX_HOME.path / "config.toml"
        return None
    ...
    def hook_files(self) -> list[Path]:
        files: list[Path] = [
            root / ".chartreux" / "hooks.toml" for root in self.project_roots
        ]
        if "user" in self.sources:
            files.append(CHARTREUX_HOME.path / "hooks.toml")
        return files
    ...
    def project_prompts_dirs(self) -> list[Path]:
        return [
            candidate
            for root in self.project_roots
            if (candidate := root / ".chartreux" / "prompts").is_dir()
        ]
    ...
    def load_user_doc(self) -> str:
        if "user" not in self.sources:
            return ""
        path = CHARTREUX_HOME.path / AGENTS_MD_FILENAME
        try:
