...
class GrepToolConfig(BaseToolConfig):
    ...
    codeignore_file: str = Field(
        default=".chartreuxignore",
        description=(
            "Name of the file to read for additional exclusion patterns. "
            "Defaults to .chartreuxignore; .vibeignore is a legacy fallback."
        ),
    )
...
class Grep(
    ...
    def _can_read(self, path: Path) -> bool:
        permission = self.resolve_permission(GrepArgs(pattern="policy", path=str(path)))
        return permission is not None and permission.permission == ToolPermission.ALWAYS

    def _collect_candidates(self, args: GrepArgs, excludes: list[str]) -> list[Path]:
        root = resolve_tool_path(args.path, self.cwd)
        if not self._can_read(root):
        ...
        for directory, dirs, files in os.walk(root, followlinks=False):
            ...
            dirs[:] = [
                name
                for name in dirs
                if self._can_read(base / name)
                and not excluded(base / name, directory=True)
            ]
            candidates.extend(
                base / name
                for name in files
    ...
    async def _filter_ripgrep_candidates(
        self, args: GrepArgs, excludes: list[str], candidates: list[Path]
    ) -> list[Path]:
        # Listing reads ignore controls, not candidate contents. Ambient rg/git
        # configuration remains disabled: it can redirect reads to arbitrary files.
        # Keep parent ignores when their existing control files pass policy.
        root = resolve_tool_path(args.path, self.cwd)
        cmd = ["rg", "--files", "--null", "--no-config", "--no-ignore-global"]
        parents = list(root.parents)
        if any(
            path.exists() and not self._can_read(path)
            for parent in parents
            for name in (".ignore", ".gitignore", ".rgignore")
            if (path := parent / name)
        ):
            cmd.append("--no-ignore-parent")
        # Only ordinary in-policy repository directories are safe to consult;
        # worktree gitdir indirection requires a separate policy-aware resolver.
        repositories = [base / ".git" for base in [root, *parents]]
        if any(
            git.exists()
            and (not git.is_dir() or not self._can_read(git / "info" / "exclude"))
        ...
        for directory, dirs, files in os.walk(root, followlinks=False):
            ...
            for name in [*dirs, *files]:
                ...
                if name in dirs and not self._can_read(path):
                    # Anchor to this search root, not every same-named directory.
                    literal = "".join(
        ...
        # Compare lexical names: resolving here can resurrect an ignored alias
        # merely because its target was listed under another name.
        visible = {self.cwd / path for path in listed.split("\0") if path}
        return [path for path in candidates if path in visible]
    ...
    def _collect_exclude_patterns(self) -> list[str]:
        ...
        configured_name = self.config.codeignore_file
        names = (
            (configured_name,)
            if "codeignore_file" in self.config.model_fields_set
            else (configured_name, ".vibeignore")
        )
        for name in names:
            codeignore_path = self.cwd / name
            if codeignore_path.is_file():
                patterns.extend(self._load_codeignore_patterns(codeignore_path))
                # The default .chartreuxignore takes precedence over its legacy
                # fallback, even when it is denied by the file policy.
                break

        return patterns

    def _load_codeignore_patterns(self, codeignore_path: Path) -> list[str]:
        if not self._can_read(codeignore_path):
            return []
        patterns = []
        try:
        ...
        try:
            ...
            try:
                ...
                await asyncio.shield(
                    kill_async_subprocess(proc, kill_process_group=False)
                )
                raise
