...
from chartreux.core.checkpoints import FileSnapshot, FileState
...
class InvokeContext:
    ...
    subagent_manager: SubagentManagementPort | None = field(default=None)
    session_dir: Path | None = field(default=None)
    launch_context: dict[str, object] | None = field(default=None)
...
class BaseTool[
    ...
    def __init__(
        ...
        *,
        ...
        self._authority_is_current: Callable[[], bool] = lambda: True
        self.state = state
        ...
        # Discovery/trust controls configuration loading, not filesystem authority.
    ...
    def _check_authority(self) -> None:
        if not self._authority_is_current():
            raise ToolPermissionError("Tool authority retired; acquire a current tool")
        if self._workspace_getter is not None:
            self.workspace = self._workspace_getter()
    ...
    def _bind_authority(
        ...
        Retained resolvers fail closed after publication retires their manager.
        This protects authorization and invoke, not arbitrary direct run calls.
        """
        ...
        def resolve_current(args: ToolArgs) -> PermissionContext | None:
            if not is_current():
                return PermissionContext(
                    ...
                    reason="Tool authority retired; acquire a current tool",
                )
            self._check_authority()
            local = resolve(args)
    ...
    def get_full_description(cls) -> str:
        ...
        file. Tools without one (e.g. MCP tools) fall back to the
    ...
    async def invoke(
        ...
        try:
            ...
            args = args_model.model_validate(raw, context={"tool_invocation": True})
    ...
    @classmethod
    def validate_tool_config(cls, values: dict[str, Any]) -> ToolConfig:
        """Validate merged tool configuration values.

        Tools with recoverable configuration errors may override this hook to
        return a safe fallback while exposing their own diagnostic.
        """
        return cls._get_tool_config_class().model_validate(values)
    ...
    def get_result_images(self, result: ToolResult) -> list[ImageAttachment] | None:
        """Return images produced by a completed tool result.

        Override in subclasses to expose attachments already present on
        *result*. This hook must be pure: it must not read files, write
        snapshots, or otherwise cause side effects.
        """
        return None
