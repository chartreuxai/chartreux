...
from chartreux.core.config import MCPHttp, MCPServer, MCPStaticAuth
...
def persistent_descriptor_context_is_safe(
    server: MCPServer, headers: Mapping[str, str]
) -> bool:
    # Stdio inherits SDK default environment even with explicit env overrides.
    # HOME/PATH can select credentials/configuration: no stdio context is proven
    # anonymous. Missing API-key values likewise do not make a declaration safe.
    return (
        isinstance(server, MCPHttp)
        and isinstance(server.auth, MCPStaticAuth)
        and not server.auth.headers
        and not server.auth.api_key_env
        and not headers
    )
    ...
    return json.dumps(
        {
            ...
            "contextPolicy": "anonymous-http/v1",
