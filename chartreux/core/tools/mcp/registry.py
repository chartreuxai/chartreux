...
from chartreux.core.tools.mcp.descriptor_cache import (
    LegacyMCPDescriptorCache,
    descriptor_cache_key,
    persistent_descriptor_context_is_safe,
)
...
from chartreux.core.tools.remote import AuthStatus, RemoteTool
from chartreux.core.utils import run_sync
from chartreux.observability.logging import logger


@dataclass(frozen=True, slots=True)
class _PrivateDescriptorContext:
    # Compared only in memory. Never hash credentials into a cache identifier.
    signature: tuple[object, ...] = field(repr=False)
    key: str = field(default_factory=lambda: f"private:{uuid4().hex}")
...
class MCPRegistry:
    ...
    def __init__(
        ...
        *,
        ...
        self._private_contexts: dict[str, _PrivateDescriptorContext] = {}
        ...
        self._authorization_owner = object()
    ...
    def configure_authorization(
        ...
        *,
        ...
        # Proxy classes capture their provider and reference. Keep raw descriptors,
        # but never reuse a class bound before authorization was replaced. Existing
        # callers retain the old reference and its provider-enforced revocation.
        self._cache.clear()
        if provider is not self._authorization_provider:
            self._authorization_owner = object()
        for alias, previous in self._authorization_refs.items():
            if references.get(alias) != previous:
                self._drop_alias_cache(alias)
                self._private_contexts.pop(alias, None)
    ...
    def _server_key(srv: MCPServer, descriptor_revision: str = "fallback") -> str:
        declaration = srv.model_dump(mode="json", exclude={"auth", "env"})
        raw = f"{json.dumps(declaration, sort_keys=True)}\0{descriptor_revision}"
    ...
    def _descriptor_key(
        ...
        if persistent_descriptor_context_is_safe(server, authorization.headers):
            self._private_contexts.pop(server.name, None)
            ...
            # Include private declaration and actual headers for legacy providers
            # too. Stdio's implicit environment and relative cwd can affect which
            # credentials/configuration its process reads; snapshot conservatively.
            signature = (
                server.model_dump_json(),
                dict(authorization.headers),
                reference.binding_id,
                authorization._descriptor_context,
                authorization.connection_revision,
                authorization.descriptor_revision,
                self._authorization_owner if not reference.binding_id else None,
                dict(os.environ) if isinstance(server, MCPStdio) else None,
                os.getcwd() if isinstance(server, MCPStdio) else None,
            )
            context = self._private_contexts.get(server.name)
            if context is None or context.signature != signature:
                context = _PrivateDescriptorContext(signature)
                self._private_contexts[server.name] = context
            key = context.key
        if self._cache_keys_by_alias.get(server.name, set()) - {key}:
            self._drop_alias_cache(server.name)
        return key
    ...
    async def get_tools_async(
        ...
        for server in servers:
            ...
            key = self._descriptor_key(server, authorization, reference)
            if server.name not in self._force_refresh:
                memory = await self._memory_hit(key, server, reference, now)
    ...
    async def _memory_hit(
        self, key: str, server: MCPServer, reference: MCPAuthorizationRef, now: datetime
        ...
        if self._descriptor_cache is not None and not key.startswith("private:"):
        ...
        if key not in self._cache:
            self._cache[key] = self._build_tools(server, record.descriptors, reference)
    ...
    async def _persistent_hit(
        ...
        if self._descriptor_cache is None or key.startswith("private:"):
            return None
    ...
    async def _discover_all(
        ...
        for (key, server, authorization, reference), discovered in zip(
            ...
            # Preserve the context captured before discovery across I/O. Only an
            # explicit authorization retry discovered under a different snapshot.
            effective_key = (
                key
                if effective_authorization == authorization
                else self._descriptor_key(server, effective_authorization, reference)
            )
            now = _utc_now()
            self._store_cache_entry(
                effective_key,
            ...
            if self._descriptor_cache is not None and not effective_key.startswith(
                "private:"
            ):
        ...
        match server.transport:
            case "streamable-http":
                return await self._discover_http(
                    cast(MCPHttp, server), authorization, reference
                )
            case "stdio":
    ...
    def _build_tools(
        ...
        async def publish_required(
            name: str, required: MCPAuthorizationRequired
        ) -> None:
            # A revoked proxy must not withdraw the replacement binding's routes
            # or discard its still-valid descriptors when its next call fails.
            if (
                provider is self._authorization_provider
                and reference == self._authorization_refs.get(name)
            ):
                await self._publish_authorization_required(name, required)
                    ...
                    proxy = create_mcp_http_proxy_tool_class(
                        ...
                        authorization_runtime=(
                            MCPHttpAuthorizationRuntime(
                                provider=provider,
                                reference=reference,
                                required_sink=publish_required,
                            )
    ...
    async def _resolve_authorization(
        ...
        if reference.kind == "oauth":
            ...
            self.mark_needs_auth(server.name)
            return required
        headers = server.http_headers() if isinstance(server, MCPHttp) else {}
        return (
            MCPAuthorizationSnapshot(
                headers=headers,
