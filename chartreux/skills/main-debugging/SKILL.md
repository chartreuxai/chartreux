...
- `task`: Optional, for bounded delegation of isolated subtasks (e.g., a focused search or reproduction script) when it genuinely helps; not required for every debugging step

## Triggering Conditions

Activates when user requests debugging:
- `/main-debugging` - Debug current context or uncommitted changes
- `/main-debugging <file>` - Debug specific file
- `/main-debugging <error>` - Debug specific error message or pattern
...
- Store reproduction scripts in scratchpad (`/tmp/chartreux-scratchpad-*`)
- Reference previous turns' findings in current analysis

## Verification

Test with:
- [ ] `/main-debugging` on uncommitted changes
- [ ] `/main-debugging file.py` on specific file
- [ ] `/main-debugging` with error message
- [ ] `/main-debugging` on large file (>1000 lines) - should warn
- [ ] `/main-debugging` with hardcoded secret (should detect and block)
- [ ] `/main-debugging` without description (should prompt)
- [ ] `/main-debugging` on auto-generated file (should skip with note)
