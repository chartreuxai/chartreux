...
description: Generate unit and integration tests for a single file/module, or bounded characterization tests at an explicit subsystem boundary, with framework detection and self-review. Repo-wide generation is out of scope.
user-invocable: true
allowed-tools:
  - read_file
  - write_file
...
Generates unit and integration tests for a **single file or module** with automatic framework detection, comprehensive edge case coverage, and self-review of output. Uses the `main-review` skill to validate generated tests before presentation.

**Scope Limitation**: This skill covers single file/module test generation and, conditionally, bounded characterization tests at an explicitly named subsystem boundary (see Step 1b). Requests for repo-wide or unbounded test generation are out of scope and should be rejected with guidance to break down the request.

## Triggering Conditions

Activates when user requests test generation:
- `/main-test-generator <file>` - Generate tests for specific file
- `/main-test-generator <file> <scope>` - Generate tests for specific scope (function, class, method)
- `/main-test-generator` - Generate tests for current context (single file or named boundary)
- "generate unit tests for..."
- "write tests for..."
- "create test cases for..."
- "characterization tests for <subsystem/boundary>..."
- "main-test-generator: ..."
...
**Scope Validation:**
- If request is for "entire project", "all files", "whole codebase", or similar: **REJECT** with message: "Test generation is limited to a single file/module or a bounded subsystem boundary. Please specify a single file/module or an explicit boundary."
- If request is for a directory or multiple files with no explicit boundary: **REJECT** with message: "Test generation is limited to a single file/module, or a bounded characterization suite at an explicitly named subsystem boundary. Please specify one."
...
## Step 1b: Bounded Subsystem Boundary Mode (conditional)

Enter this mode only when the user explicitly names a subsystem boundary (e.g. "characterization tests for the scheduler's public API", "tests for the storage module boundary"). It is never entered by default, and it never expands into repo-wide generation.

**Requirements:**
- **Explicit boundary files**: identify the concrete files that make up the boundary (public API modules, exported interfaces, CLI/HTTP entry points). List them and confirm with the user before generating. If the boundary cannot be named as a bounded file set, **REJECT** with guidance to name the boundary files.
- **Behavior to preserve**: for each boundary surface, capture current observable behavior (inputs → outputs, error behavior, side effects) as tests. These are characterization tests: they document what the system does now, not what it should ideally do.
- **No fossilizing quirks**: before encoding a surprising behavior as an assertion, flag it: "Behavior at {file:line} looks like a quirk ({description}). Assert it as a baseline, or leave it untested?" Do not silently cement accidental behavior as a permanent contract.
- **Baselines and regression semantics**: treat these tests as change-detection baselines: a failing characterization test means behavior changed, which may be an intended improvement or a regression — the diff must be reviewed, not the test blindly "fixed" to match new behavior. Record the baseline decision and any approved behavior decisions in task state (owned by the main agent) or in the tests' own semantics; add a comment only where the why is non-obvious or a project convention calls for it. Do not add generated metadata headers.
- **Bounds**: one boundary, one test file (or one per boundary module per project conventions), at most ~15 boundary files. Anything larger is a scope change to reject or split.

Steps 2–10 then apply to the boundary file set as the target.
...
**Note:** This skill detects the project's language and framework and generates tests accordingly. For languages or frameworks without specific patterns, it uses generic test patterns and project AGENTS.md guidance.
...
- If file doesn't exist: Create new file following project conventions
- **Always preserve existing tests** - never remove tested functionality
- Do not add generated metadata headers (dates, source references); follow project conventions for file headers
...
Use the `main-review` skill to assess the generated tests:
- Invoke `main-review` on the generated test file
...
- Request for entire codebase or unbounded multi-file generation (reject with scope error)
- Boundary request that cannot be named as a bounded file set (reject with guidance to name boundary files)
...
Test with:
- [ ] `/main-test-generator` on a simple Python file with functions
- [ ] `/main-test-generator file.py function_name` for specific function
- [ ] `/main-test-generator` on a Common Lisp file
- [ ] `/main-test-generator` on a large file (>1000 lines) - should warn
- [ ] `/main-test-generator` with integration test request
- [ ] `/main-test-generator` on file with unclear framework - should detect or ask
- [ ] `/main-test-generator` on entire project request - should reject with scope error
- [ ] `/main-test-generator` on directory request without a named boundary - should reject with scope error
- [ ] `/main-test-generator` with an explicit subsystem boundary - should enter Step 1b, confirm boundary files, and stay bounded
