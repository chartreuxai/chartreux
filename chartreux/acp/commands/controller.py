...
from chartreux.acp.commands.registry import AcpCommand, AcpCommandKind
from chartreux.acp.exceptions import from_app_server_error
...
class AcpCommandController:
    ...
    async def _proxy_setup(self, session: AcpSession, arguments: str) -> PromptResponse:
        ...
        env_file = get_chartreux_home() / ".env"
        if resolved_value is None:
            message = (
                f"Removed `{key}` from {env_file}\n\n"
                "Please start a new chat for changes to take effect."
            )
        else:
            message = (
                f"Set `{key}={resolved_value}` in {env_file}\n\n"
                "Please start a new chat for changes to take effect."
            )
        return await self._reply(session, message)
