...
Verifies that every ``from <module> import <name>`` across the chartreux source tree
...
def _is_chartreux_module(module: str) -> bool:
    return module == "chartreux" or module.startswith("chartreux.")


def _missing_dep_name(exc: Exception) -> str | None:
    """Return the missing module name if *exc* is a non-Chartreux dependency issue."""
    if isinstance(exc, ModuleNotFoundError) and exc.name is not None:
        return exc.name if not _is_chartreux_module(exc.name) else None
    return None
...
def _resolve_relative(
    file_path: Path, chartreux_pkg: Path, level: int, name: str | None
) -> str | None:
    rel = file_path.relative_to(chartreux_pkg).with_suffix("")
    ...
    if name:
        base += name.split(".")
    return ".".join(["chartreux", *base]) if base else "chartreux"


class ImportVisitor(ast.NodeVisitor):
    def __init__(self, file_path: Path, chartreux_pkg: Path) -> None:
        self.file_path = file_path
        self.chartreux_pkg = chartreux_pkg
        self.imports: list[tuple[str, str, int]] = []

    def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
        if node.level and node.level > 0:
            mod = _resolve_relative(
                self.file_path, self.chartreux_pkg, node.level, node.module
            )
            if mod is None:
                return
        else:
    ...
    for root in [chartreux_pkg, repo / "tests"]:
        ...
        for py in root.rglob("*.py"):
            ...
            try:
                ...
                continue
            visitor = ImportVisitor(py, chartreux_pkg)
            visitor.visit(tree)
            if visitor.imports:
...
def _check_pydantic_models() -> tuple[dict[str, list[str]], int]:
    ...
    for mod_name in sorted(sys.modules):
        if not _is_chartreux_module(mod_name):
            continue
...
def check_all() -> int:
    ...
    chartreux_pkg = repo / "chartreux"
    sys.path.insert(0, str(repo))

    pairs: dict[tuple[str, str], set[Path]] = {}
    star_modules: dict[str, set[Path]] = {}
    for file_path, imports in collect_imports(repo, chartreux_pkg).items():
        for mod, name, _lineno in imports:
            if not _is_chartreux_module(mod):
                continue
            if name == "*":
