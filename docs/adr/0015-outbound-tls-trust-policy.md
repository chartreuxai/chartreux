...
Every Chartreux-owned connection that verifies TLS certificates must respect the
...
during bootstrap, onboarding, and browser-facing MCP OAuth authorization.
Chartreux-owned clients must not silently fall back to a library's default SSL
context. Connections made by an external browser or subprocess are outside this
boundary.
...
after authentication makes the authentication connection fail before Chartreux can
start.

## Agent Guidance

- Use Chartreux's shared HTTP/TLS helpers for Chartreux-owned clients.
...
- A dependency cannot accept Chartreux's SSL context or equivalent trust policy.
