...
Chartreux should evolve toward pragmatic hexagonal architecture: core behavior depends on stable models and ports, while UI, filesystem, network, provider SDKs, subprocesses, and protocols live at the edges.
...
Chartreux is an interactive CLI agent. Slow startup, broad edits, and tangled boundaries are user-visible. Architecture should make the common path fast and make change local.
...
- Keep cross-surface session, runtime, resource, and protocol orchestration in
  `chartreux.app_server`; delivery surfaces consume its public client API.
