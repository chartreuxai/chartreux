...
`chartreux/core` supplies reusable private engine implementations: agent loop,
tools, LLM backends, config, sessions, skills, hooks, and shared domain models.
`chartreux/app_server` is the harness and runtime composition root that owns those
objects for delivery surfaces.

Delivery surfaces adapt that engine:

- `chartreux/cli` owns the Textual app, terminal UX, widgets, slash-command presentation, and local interactive affordances. Model-visible shell execution crosses the app server.
- `chartreux/acp` owns thin Agent Client Protocol translation and ACP-specific presentation updates over the app-server client API.
- `chartreux/setup` owns first-run and onboarding flows.
...
- Add user-interface behavior in `chartreux/cli`, not `chartreux/core`.
- Add protocol translation in `chartreux/acp`, not `chartreux/core`.
...
- Implementing a feature requires `chartreux/core` to import Textual, ACP schema objects, or setup UI code.
