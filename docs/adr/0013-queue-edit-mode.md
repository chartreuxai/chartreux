...
callbacks from `ChartreuxApp` through `ChatInputContainer`:
...
`ChartreuxApp` owns the highlighted widget reference and the `queue-selected` CSS
...
- Keep the `queue-selected` CSS class in `ChartreuxApp`, which owns the selected
  widget reference.
